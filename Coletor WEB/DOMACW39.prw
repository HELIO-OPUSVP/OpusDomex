...
±±∫Programa  ≥DOMACW39  ∫Autor  ≥Microsiga           ∫ Data ≥  10/02/19   ∫±±
...
User Function DOMACW39(__cFilial)
	...
	Private cLocTransf  := If(__cFilial=='01'.OR. cFilAnt == '02',GetMV("MV_XXLOCPR"),GetMV("MV_XLOCTRA"))

	Private cEndProDom	:= If(__cFilial=='01' .OR. cFilAnt == '02',"97PROCESSO"       ,"95TRANSFERENCIA"  )
	Private lImprimeETq :=.T.
	Private nWebPx:= 1.5
	
	Private cPush:= "background-color: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 1,stop: 0 gray, stop: 0.4 white,stop: 1 gray);color: #171717; font: bold "+cvaltochar(12*nWebPx)+"px Arial;"+;
	"background-size:15% 60%;background-position: 1% 5%;background-repeat:no-repeat ;border-radius: 6px;}"
	Private cPressed:= "background-color: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 1,stop: 0 gray, stop: 0.4 white,stop: 1 blue);color: #171717; font: bold "+cvaltochar(12*nWebPx)+"px Arial;"+;
	"background-size:15% 60%;background-position: 1% 5%;background-repeat:no-repeat ;border-radius: 6px;}"
	Private cHover:="background-color: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 1,stop: 0 gray, stop: 0.4 white,stop: 1 Black);color: #171717; font: bold "+cvaltochar(12*nWebPx)+"px Arial;"+;
	"background-size:15% 60%;background-position: 1% 5%;background-repeat:no-repeat ;border-radius: 6px;}"
	...
	DEFINE MSDIALOG oDlg0 TITLE OemToAnsi("Data-Pagto OP por Picklist") FROM 0,0 TO 450,302 PIXEL OF oMainWnd

	nLin += 10

	@ nLin  ,nCol2	SAY oTxt1 Var   'Data:'  SIZE 25*nWebPx,20 PIXEL OF oDlg0
	oTxt1:oFont   := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)

	@ nLin-10, nCol2 + 020  MSGET oData    VAR dData    Picture "@D"  SIZE 60*nWebPx,20*nWebPx PIXEL OF oDlg0
	oData:oFont   := TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)

	nLin += 40*nWebPx
	@ nLin , nCol1 + 005 BUTTON oBotao1 PROMPT "-1"     ACTION Processa( {|| Botao(1) } ) SIZE 40*nWebPx,30*nWebPx PIXEL OF oDlg0
	@ nLin , nCol1 + 075 BUTTON oBotao2 PROMPT "+1"     ACTION Processa( {|| Botao(2) } ) SIZE 40*nWebPx,30*nWebPx PIXEL OF oDlg0


	@ 120*nWebPx,045 Button oBotao3 PROMPT "Ok" Size 40*nWebPx,13*nWebPx Action Close(oDlg0) Pixel
...
//	"QPushButton:pressed { background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #dadbde, stop: 1 #f6f7fa); }"
	...
	If (Upper(GetEnvServ()) == 'HOMOLOGACAO') .OR. .T.
		...
		cQryGrps += ENTER + " JOIN ( SELECT D4_FILIAL,D4_OP FROM "+retsqlname("SD4")+" SD4 WHERE SD4.D_E_L_E_T_ ='' AND D4_FILIAL = '"+__cFilial+"' AND D4_LOCAL = '97' AND D4_QUANT > 0 "
	...
	@ 00,00 SCROLLBOX oDlg1 VERTICAL BORDER SIZE 133*nWebPx, 118*nWebPx PIXEL OF oMainWnd

	@ nLin  ,nCol2	SAY oTxt2 Var   'Data: '  + DTOC(dData) SIZE 50*nWebPx,15*nWebPx PIXEL OF oDlg1
	...
	For nP := 1 To Len(aGruposPrd)
		@ nLin, 05 BUTTON &("oBtng" + Strzero(nP,2)) PROMPT aGruposPrd[nP]     ACTION Processa( {||   BuscaEmpOp(__cFilial) /*U_DOMACW39()*/} ) SIZE nLargBut*nWebPx,nAltuBut PIXEL OF oDlg1
	...
	If !Empty(Alltrim(cGrupoProd))
		...
		If Len(aImpEtq)	 > 0
			...
			For ns:=1 To Len(aImpEtq)
				If lImprimeETq
					U_EtqNOPW39(aImpEtq[ns][1],aImpEtq[ns][2],aImpEtq[ns][3],aImpEtq[ns][4])
				EndIf
			Next ns
		Endif
		...
		If !TEMPS->(EOF())
			While !TEMPS->(EOF())
				...
				DEFINE MSDIALOG oDlg2 TITLE OemToAnsi("Produto-Pagto OP por Picklist") FROM 0,0 TO 450,302 PIXEL OF oMainWnd

				@ nLin  ,nCol2	SAY oTxt1 Var   'Produto: ' + cCodProdut  SIZE 100*nWebPx,10*nWebPx PIXEL OF oDlg2
				oTxt1:oFont   := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)
				nLin += 13*nWebPx

				@ nLin  ,nCol2	SAY oTxt2 Var   'DescriÁ„o:' SIZE 50*nWebPx,10*nWebPx PIXEL OF oDlg2
				oTxt2:oFont   := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)
				
				nLin += 08*nWebPx

				@ nLin  ,nCol2	SAY oTxt3 Var   cDescrProd 	SIZE 150*nWebPx,10*nWebPx PIXEL OF oDlg2
				oTxt3:oFont   := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)
				
				nLin += 13*nWebPx
				If !Empty(cDescrPro2)
					@ nLin  ,nCol2	SAY oTxt31 Var   cDescrPro2 	SIZE 150*nWebPx,10*nWebPx PIXEL OF oDlg2
						oTxt31:oFont   := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)
					nLin += 11*nWebPx
				EndIf
				iF 	nQtdSaldo < nQtdaSep  .Or.  nQtdSldEnd < nQtdaSep
					@ nLin  ,nCol2	SAY oTxt4 Var   'Qtd.Saldo: ' + Transform(nQtdSaldo,"@E 999,999,999.9999")  SIZE 150*nWebPx,10*nWebPx COLORS CLR_RED,CLR_WHITE PIXEL OF oDlg2
					oTxt4:oFont   := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)
					nLin += 12*nWebPx
				Else
					@ nLin  ,nCol2	SAY oTxt4 Var   'Qtd.Saldo: ' + Transform(nQtdSaldo,"@E 999,999,999.9999")  SIZE 150*nWebPx,10*nWebPx PIXEL OF oDlg2
					oTxt4:oFont   := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)
					nLin += 12*nWebPx
				EndIf
					
				@ nLin  ,nCol2	SAY oTxt4 Var   'Qtd.a separar: ' + Transform(nQtdasep,"@E 999,999,999.9999")  SIZE 150*nWebPx,10*nWebPx PIXEL OF oDlg2
				oTxt4:oFont   := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)
				nLin += 12*nWebPx


				oGetDados  := (MsNewGetDados():New( 70*nWebPx, 00 , 130*nWebPx ,109*nWebPx,/*GD_UPDATE+GD_DELETE*/0,"AlwaysTrue" ,"AlwaysTrue", /*inicpos*/,/*aCpoHead*/,/*nfreeze*/,9999 ,/*Ffieldok*/,/*superdel*/,/*delok*/,oDlg2,aHeader,aCols))
				...
				@ 134*nWebPx,005 Button oBotao1 PROMPT "Coletar" 	Size 30*nWebPx,13*nWebPx Action (ColetaProd(cCodProdut,cGrupoProd,dData,nQtdEmp,nQtdasep,aEnderecos,__cFilial)) Pixel OF oDlg2
				@ 134*nWebPx,050 Button oBotao3 PROMPT "Anterior" 	Size 30*nWebPx,13*nWebPx Action (nOpcSel:=1,Close(oDlg2)) Pixel OF oDlg2
				@ 134*nWebPx,095 Button oBotao2 PROMPT "Proximo" 	Size 30*nWebPx,13*nWebPx Action (nOpcSel:=2,Close(oDlg2)) Pixel OF oDlg2
...
Static Function ColetaProd(cCodProdut,cGrupoProd,dData,nQtdEmp,nQtdasep,aEnderecos,__cFilial)
	...
	If TMPOPS->(!EOF())
		...
		While TMPOPS->(!EOF())
			...
			If (Upper(GetEnvServ()) == 'HOMOLOGACAO') .OR. .T.
				AADD(aOpsApagar,{TMPOPS->D4_OP,(TMPOPS->QTDORI - TMPOPS->SUM_D3QTD),cRpcFil})
			Else
				AADD(aOpsApagar,{TMPOPS->D4_OP,(TMPOPS->QTDORI - TMPOPS->SUM_D3QTD)})
			EndIf
			TMPOPS->(DbSkip())
		EndDo
		/*else
	...
	If Len(aEnderecos) > 0
		...
		For nK := 1 To Len(aEnderecos)
			If aEnderecos[nK][2] >= (nSaldoSD4  - nQtJaSepar)
				...
				PagaOpsProd(_cProdEmp,_cDescric,_nQtdEmp,_cEnderec,(nSaldoSD4  - nQtJaSepar),aOpsApagar,_cLocPadP)
...
Static Function PagaOpsProd(_cProdEmp,_cDescric,_nQtdEmp,_cEnderec,nSaldoSD4,aOpsApagar,_cLocPadP)
	Local nIncLin           := -15*nWebPx
	...
	@ 018*nWebPx+nIncLin,001 To 057*nWebPx+nIncLin,115 Pixel Of oMainWnd PIXEL

	@ 020*nWebPx+nIncLin,005 Say oTxtLabelOP  Var "Produto sugerido"                            Pixel Of oTelaOP
	@ 027*nWebPx+nIncLin,005 Say oTxtProdEmp  Var "CÛdigo: "+ _cProdEmp                         Pixel Of oTelaOP
	@ 027*nWebPx+nIncLin,077 Say oTxtQtdEmp   Var "Qtd: " + TransForm(_nQtdEmp,"@E 999,999.99") Pixel Of oTelaOP
	@ 033*nWebPx+nIncLin,005 Say oTxtDescric  Var "DescriÁ„o: "+ _cDescric Size 110,15          Pixel Of oTelaOP
	@ 048*nWebPx+nIncLin,005 Say oTxtEnderec  Var "EndereÁo: " + _cEnderec                      Pixel Of oTelaOP

	@ 130*nWebPx+nIncLin,005 Say oSaldoSD4    Var "Saldo: " + Alltrim(Transform(nSaldoSD4 - nQTransfOk ,"@E 999,999,999.9999")) Pixel Of oTelaOP
	...
	@ nLin-2+nIncLin,045 MsGet oGetEnd Var cGetEnd Valid fValidEnd() Size 70*nWebPx,10*nWebPx  Pixel Of oTelaOP
	...
	@ nLin-(2*nWebPx)+nIncLin,045 MsGet oGetEtiq Var _cEtiqueta  Size 70*nWebPx,10*nWebPx Valid IIF(!Empty(Alltrim(_cEtiqueta)),IIF(ValidaEtiq(),ValidQtd(aOpsAPagar,_nQtd),.F.),.T.) Pixel Of oTelaOP
	...
	@ nLin-(2*nWebPx)+nIncLin,045 MsGet oGetQtd  Var nQtEtqLida When .F. Picture "@E 9,999,999.9999" Size 70*nWebPx,10*nWebPx  Pixel Of oTelaOP
	oTxtQtd:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	oGetQtd:oFont := TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)

	nLin+= 20*nWebPx

	@ nLin,085 Button "Fechar"   Size 25*nWebPx,15*nWebPx Action Close(oTelaOp) Pixel Of oTelaOP
...
Static Function fTelaEti()
	...
	Define MsDialog oTelaEti Title OemToAnsi("CorreÁ„o Etiqueta") From 0,0 To 450,302 Pixel of oMainEti PIXEL
	...
	@ 027*nWebPx,005 Say oTxtProdEti  Var "CÛdigo: "   + _cProdEti Pixel Of oTelaEti
	@ 027*nWebPx,077 Say oTxtQtdEti   Var "Qtd: "      + TransForm(_nQtdEti,"@E 999,999.99") Pixel Of oTelaEti
	@ 033*nWebPx,005 Say oTxtDescEti  Var "DescriÁ„o: "+ _cDescEti Size 110*nWebPx,15*nWebPx Pixel Of oTelaEti
	@ 048*nWebPx,005 Say oTxtEndeEti  Var "EndereÁo: " + _cEndeEti Pixel Of oTelaEti
									...
									If SD3->( dbSeek( xFilial() + _cDoc ) )
										...
										If  nQTransfOk == aOpsApagar[nR][2] //.Or. nQEtqLidas == nK
											...
											If aOpsApagar[nR][2] > 0 .And. lImprime
												U_EtPgOPW39(_cNumOP,XD1->XD1_COD,SB1->B1_DESC,nQTransfOk,.f./*lparcial*/,nR,Len(aOpsApagar),_cFilialOP)
												...
												If lSilk
													...
													U_EtqPWSilk(_cNumOP,cPnOpSilk,cDescSilk,nQTransfOk,.f./*lparcial*/,SD4->D4_FILIAL)
...
User Function EtPgOPW39(cNumOp,cCodProd,cDescProd,nQtdProd,lParcial,nSeqEtq,nQtdTotEtq,cFilialOP)
	Local _cPorta    := "LPT1"
...
User Function EtqNOPW39(cNumOp,cCodProd,cDescProd,cFilialOP)
	Local _aAreaGER  := GetArea()
	Local cFila := ""
	IF cFilAnt == "01"
		 cFila := "000023"
...
User Function EtqPWSilk(cNumOp,cCodProd,cDescProd,nQtdProd,lParcial,cFilialOP)
	Local _cPorta    := "LPT1"
	Local _aAreaGER  := GetArea()
