...
//ณ Tela do Coletor													    ณ
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	DEFINE MSDIALOG oInv TITLE OemToAnsi("Contagem: " ) FROM 0,0 TO 450,302 PIXEL OF oMainWnd PIXEL

	nLin := 30*nWebPx
	@ nLin, 020	SAY oTexto1 Var 'Informe a contagem:'    SIZE 100*nWebPx,10*nWebPx PIXEL
	oTexto1:oFont := TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)

	nLin += 20*nWebPx
	@ nLin, 020	SAY oTexto2 Var 'Contagem:'    SIZE 40*nWebPx,10*nWebPx PIXEL
	oTexto2:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)

	@ nLin, 060 COMBOBOX oCombo2  VAR cContagem ITEMS aContagem    SIZE 45*nWebPx,10*nWebPx VALID VLDCONT() PIXEL
	oCombo1:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	
	nLin += 20*nWebPx
	@ nLin, 020 SAY oTexto2 Var "Etiqueta" SIZE 100*nWebPx,10*nWebPx PIXEL
	oTexto2:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	
	nLin += 10*nWebPx
	@ nLin,020 MSGET oGetCont Var cContEtq WHEN lContagem VALID VLDSZI() Size 080*nWebPx,10*nWebPx Pixel
	oGetCont:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	
	nLin += 40*nWebPx
	@ nLin, 10 Button _oBtn Prompt "Confirmar" ACTION Processa( {|| oInv:End()} ) SIZE nLargBut,nAltuBut PIXEL OF oInv
...
Static Function fOkTela()
	...
	DEFINE MSDIALOG oInv TITLE OemToAnsi("Inventแrio GERAL " + DtoC(GetMV("MV_XXDTINV"))) FROM 0,0 TO 450,302 PIXEL OF oMainWnd PIXEL

	@ 005,005 Say oTxtEnd    Var "Endere็o " Pixel Of oInv
	@ 005,045 MsGet oGetEnd  Var cEndereco Valid ValidEnd() Size 70*nWebPx,10*nWebPx Pixel Of oInv
	oTxtEnd:oFont := TFont():New('Arial',,20,,.T.,,,,.T.,.F.)
	oGetEnd:oFont := TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)

	@ 020*nWebPx,005 Say oTxtEtiq   Var "Etiqueta " Pixel Of oInv
	@ 020*nWebPx,045 MsGet oGetEtiq Var cEtiqueta  Size 70*nWebPx,10*nWebPx Valid ValidEtiq() Pixel Of oInv
	oTxtEtiq:oFont:= TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	oGetEtiq:oFont:= TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)

	@ 035*nWebPx,005 Say oTxtProd   Var "Produto "  Pixel Of oInv
	@ 035*nWebPx,045 MsGet oGetProd Var _cProduto When .F. Size 70*nWebPx,10*nWebPx  Pixel Of oInv
	oTxtProd:oFont:= TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	oGetProd:oFont:= TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)

	@ 050*nWebPx,005 Say oTxtQtd    Var "Quant. " Pixel Of oInv
	@ 050*nWebPx,045 MsGet oGetQtd  Var _nQtd Valid ValidQtd()      Picture "@E 9,999,999.9999" Size 70*nWebPx,10*nWebPx  Pixel Of oInv
	oTxtQtd:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	oGetQtd:oFont := TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)

	@ 065*nWebPx,005 Say oTxtEnd    Var "Ultimas Etiquetas" Pixel Of oInv
	oTxtEnd:oFont:= TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)

	@ 075*nWebPx,005 Say oUltEti1   Var cUltEti1         Pixel Of oInv
	oUltEti1:oFont:= TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	@ 085*nWebPx,005 Say oUltEti2   Var cUltEti2         Pixel Of oInv
	oUltEti2:oFont:= TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	@ 095*nWebPx,005 Say oUltEti3   Var cUltEti3         Pixel Of oInv
	oUltEti3:oFont:= TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)

	@ 110*nWebPx,050 Say oContad   Var Transform(nContad,"@E 999,999")  Size 70*nWebPx,20*nWebPx       Pixel Of oInv
	oContad:oFont:= TFont():New('courier',,35*nWebPx,,.T.,,,,.T.,.F.)

	@ 130*nWebPx,070 Button _oBtn Prompt "Sair" Size 40*nWebPx,15*nWebPx Action Close(oInv) Pixel Of oInv
    cCSSBtN1 := "QPushButton{"+cPush+;
	"QPushButton:pressed {"+cPressed+;
	"QPushButton:hover {"+cHover
	_oBtn:setCSS(cCSSBtN1)
