...
±±ºPrograma  ³DOMACD35  ºAutor  ³Jonas Pereira      º Data ³  10/09/18   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Pagamento de Perdas - Rotina original                      º±±
±±ºPrograma  ³  ºAutor  ³Helio Ferreira      º Data ³ 			   º±±
±±º          ³                                                            º±±
...
User Function DOMACW35()
	...
	Define MsDialog oTelaOP Title OemToAnsi(cTitulo + DtoC(dDataBase) ) From 0,0 To 450,302 Pixel of oMainWnd PIXEL //293

	nLin := 005*nWebPx
	@ nLin,002   Say oTxtOP    Var "OP(perda)" Pixel Of oTelaOP
	@ nLin-2*nWebPx,050 MsGet oGetOP  Var _cNumOP Valid fValidaOP() Size 65*nWebPx,10*nWebPx Pixel Of oTelaOP
	oTxtOP:oFont := TFont():New('Arial',,16*nWebPx,,.T.,,,,.T.,.F.)
	oGetOP:oFont := TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)

	@ 016*nWebPx,001 To 057*nWebPx,115*nWebPx Pixel Of oMainWnd PIXEL
	@ 017*nWebPx,005 Say oTxtLabelOP  Var "Produto " Pixel Of oTelaOP
	@ 024*nWebPx,005 Say oTxtProdEmp  Var "Código: " + _cProdEmp Pixel Of oTelaOP
	@ 024*nWebPx,077 Say oTxtQtdEmp   Var "Qtd: " + TransForm(_nQtdEmp,"@E 999,999.99") Pixel Of oTelaOP
	@ 030*nWebPx,005 Say oTxtDescric  Var "Descrição: " + _cDescric Size 110*nWebPx,15*nWebPx Pixel Of oTelaOP
	@ 045*nWebPx,005 Say oTxtEnderec  Var "Endereço: "+ _cEnderec Pixel Of oTelaOP
	@ 115*nWebPx,005 Say oSaldoSZA    Var "Saldo: " + Alltrim(Transform(nSaldoSZA,"@E 999,999,999.9")) Pixel Of oTelaOP

	oSaldoSZA:oFont := TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)
	oTxtLabelOP:oFont := TFont():New('Arial',,14*nWebPx,,.T.,,,,.T.,.F.)
	oTxtProdEmp:oFont := TFont():New('Arial',,14*nWebPx,,.T.,,,,.T.,.F.)
	oTxtDescric:oFont := TFont():New('Arial',,14*nWebPx,,.T.,,,,.T.,.F.)
	oTxtEnderec:oFont := TFont():New('Arial',,17,,.T.,,,,.T.,.F.)
	...
	oTxtEtiq:oFont:= TFont():New('Arial',,20,,.T.,,,,.T.,.F.)
	...
	@ nLin-2*nWebPx,045 MsGet oGetQtd  Var _nQtd Valid fOkQtd() Picture "@E 9,999,999.99" Size 70*nWebPx,10*nWebPx  Pixel Of oTelaOP
	oTxtQtd:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
	oGetQtd:oFont := TFont():New('Arial',,20*nWebPx,,.T.,,,,.T.,.F.)

	nLin+= 13*nWebPx
	@ nLin,070 Button  _oBtn Prompt "Cancelar" Size 40*nWebPx,15*nWebPx Action Close(oTelaOp) Pixel Of oTelaOP
...
Static Function fTelaEti()
	...
	nLin+= 60*nWebPx
	@ nLin  ,005 Say oTxtQtdEti    Var "Quant. " Pixel Of oTelaEti
	...
	nLin+= 20*nWebPx
	@ nLin,070 Button _oBtn Prompt "Cancelar" Size 40*nWebPx,15*nWebPx Action Close(oTelaEti) Pixel Of oTelaEti
	cCSSBtN1 := "QPushButton{"+cPush+;
		"QPushButton:pressed {"+cPressed+;
		"QPushButton:hover {"+cHover
	_oBtn:setCSS(cCSSBtN1)
...
Static Function fQtdEti()
	...
	If _nQtdEti >0
		If _nQtdEti <= SB2->B2_QATU
			...
			MsUnlock()
		Else
			U_MsgColetor("Não foi possível realizar o ajuste .Inventarie todo o saldo do produto "+AllTrim(XD1->XD1_COD)+" saldo atual :"+TransForm(SB2->B2_QATU,"@E 999,999.99")+".")
...
Static Function fValidaOP()
	...
	If Len(Alltrim(_cNumOP)) == 6
		Return fValidaDoc()
	Else
		U_MsgColetor("Não é permitido o pagamento pela OP")
		Return .F.
	EndIf
	...
	If SC2->( dbSeek(xFilial("SC2")+Subs(_cNumOP,1,11)))
		If SC2->C2_QUANT <> SC2->C2_QUJE
			...
			If SZA->( dbSeek(xFilial()+Subs(_cNumOP,1,11)))
				While xFilial("SZA")+Subs(_cNumOP,1,11) == SZA->ZA_FILIAL+SZA->ZA_OP
					If SB1->( dbSeek( xFilial() + SZA->ZA_PRODUTO ) )
						If SZA->ZA_SALDO > 0
							...
							Else
								aadd(_aDados,{SB1->B1_COD,SB1->B1_DESC,' Sem Endereço',SZA->ZA_SALDO})
							EndIf
							//EndIf
						EndIf
					EndIf
				...
				End
			Else
				U_MsgColetor("Não foram encontrados apontamentos de perda para esta OP.")
			EndIf
		Else
	...
	If !Empty(_cEtiqueta)
		...
		*/
			...
			FIBRAS
			    ...
			    U_MsgColetor("Não é permitido o pagamento de PERDA de Fibras por esta ferramenta. Utilize a ferramenta específica para envio deste material para o endereço '01CORTE'.")
			   _cEtiqueta  := Space(_nTamEtiq)
				oGetEtiq:Refresh()
				_Retorno := .F.
				ENDIF
				...
				If _Retorno
					...
					If Empty(_cNumZADOC)
						...
						If SZA->( dbSeek( xFilial() + Subs(_cNumOP,1,11) + XD1->XD1_COD ) )
							...
							End
							If !bSldSZA
								U_MsgColetor('Não existe saldo de PERDA deste produto para esta OP a ser pago.')
								_cEtiqueta := Space(_nTamEtiq)
								oGetEtiq:Refresh()
								_Retorno   := .F.
							EndIf
						...
						If SZA->( dbSeek( xFilial() + _cNumZADOC ) )
							...
							Else
								If SZA->ZA_SALDO > 0
									bSldSZA := .T.
								Else
									U_MsgColetor('Perda já atendida.')
									_cEtiqueta := Space(_nTamEtiq)
									oGetEtiq:Refresh()
									_Retorno   := .F.
								EndIf
							...
							If SZE->( dbSeek(xFilial() + _cNumZADOC))
								...
								Else
									If SZE->ZE_SALDO > 0
										bSldSZA := .T.
									Else
										U_MsgColetor('Perda de Fornecedor já atendida.')
										_cEtiqueta := Space(_nTamEtiq)
										oGetEtiq:Refresh()
										_Retorno   := .F.
									EndIf
							...
							Else
								...
								If SZX->( dbSeek( xFilial() + _cNumZADOC ) )
									If SZX->ZX_PRODUTO <> XD1->XD1_COD
										U_MsgColetor('Produto diferente da Solicitação de MC.')
										_cEtiqueta := Space(_nTamEtiq)
										oGetEtiq:Refresh()
										_Retorno   := .F.
									Else
										If SZX->ZX_SALDO > 0
											bSldSZA := .T.
										Else
											U_MsgColetor('Solicitação de MC já atendida.')
											_cEtiqueta := Space(_nTamEtiq)
											oGetEtiq:Refresh()
											_Retorno   := .F.
										EndIf
					...
					If _Retorno
						...
						If SB2->( dbSeek( xFilial() + XD1->XD1_COD + XD1->XD1_LOCAL ) )
							If SB2->B2_QATU < XD1->XD1_QTDATU
								...
								//_lOkProc:=fTelaEti()
								U_MsgColetor('Saldo físico (sistêmico) menor que o saldo da etiqueta. Inventarie o produto.')
								_lOkProc := .F.
							EndIf
						EndIf
					...
					//EndIf
					//Else
					//	U_MsgColetor('Não existe apontamento de PERDA do produto '+Alltrim(XD1->XD1_COD)+' para esta OP.')
					//	_Retorno := .F.
					//EndIf
				Else
...
Static Function fOkQtd()
	...
	If _nQtd > 0
		If U_uMsgYesNo("Quantidade :"+TransForm(_nQtd,"@E 9,999,999.99"),"Confirma o pagamento ?")
			...
			If cSZASZE == "SZX"  // Pagamento Solicitação de Materiais de Consumo
				_cLote:=If(Empty(XD1->XD1_LOTECTL),"LOTE1308",XD1->XD1_LOTECTL)
				...
				If _nQtd <= XD1->XD1_QTDATU
					If _nQtd <= nSalSZASZE
						...
						EndIf
					Else
						U_MsgColetor('Quantidade superior a Solicitação de MC.')
						_Retorno := .F.
					EndIf
				Else
					U_MsgColetor('Quantidade superior a da etiqueta. Se estiver correta, favor inventariar o material.')
	...
	If SZA->( dbSeek(xFilial()+_cNumZADOC))
		...
		If SC2->( dbSeek(xFilial()+SZA->ZA_OP))
			If !Empty(SC2->C2_DATRF)
				U_MsgColetor("Ordem de Produção   encerrada em        " + DtoC(SC2->C2_DATRF)+".")
				_Retorno := .F.
			Else
				_cNumOP        := SZA->ZA_OP
				...
				If SC2->C2_QUANT <> SC2->C2_QUJE
					If SB1->( dbSeek( xFilial() + SZA->ZA_PRODUTO ) )
						...
						EndIf
					Else
						U_MsgColetor("Produto do apontamento de perda não encontrado.")
						_Retorno := .F.
					EndIf
				Else
					U_MsgColetor("OP já encerrada.")
				...
				If SC2->C2_QUANT <> SC2->C2_QUJE
					If SB1->( dbSeek( xFilial() + SZE->ZE_PRODUTO ) )
						If SZE->ZE_SALDO > 0
							...
							Else
								aadd(_aDados,{SB1->B1_COD,SB1->B1_DESC,' Sem Endereço',SZE->ZE_SALDO})
							EndIf
						Else
							U_MsgColetor("Perda de Fornecedor já atendida.")
							_Retorno := .F.
			...
			If SZX->( dbSeek(xFilial()+_cNumZADOC))
				If SB1->( dbSeek( xFilial() + SZX->ZX_PRODUTO ) )
					...
					If SZX->ZX_SALDO > 0
						...
						Else
							aadd(_aDados,{SB1->B1_COD,SB1->B1_DESC,' Sem Endereço',SZX->ZX_SALDO})
						EndIf
					Else
						U_MsgColetor("Perda já atendida.")
						_Retorno := .F.
					EndIf
				Else
					U_MsgColetor("Produto da Solicitação de MC não encontrado.")
					_Retorno := .F.
				EndIf
			Else
				If !Empty(_cNumZADOC)
					U_MsgColetor("Documento " + _cNumZADOC + " de apontamento de peda não encontrado.")
					_Retorno := .F.
				Else
					_Retorno := .T.
				EndIf
