...
±±ºPrograma  ³DOMACW41  ºAutor  ³Michel A. Sander    º Data ³  01/11/17   º±±
...
_cEndDest := "95TRANSFERENCIA" // PADR("01CORTE",15)
_cLocDest := "95"
_cLocOrig := "01"

DEFINE MSDIALOG oTrans TITLE OemToAnsi("Entrega de Bobina Filial MG") FROM 0,0 TO 450,302 PIXEL OF oMainWnd PIXEL
...
@ 031*nWebPx,045 MsGet oGetEndDEST  Var _cEndDest When .F. Valid fValEnd() Size 70*nWebPx,10*nWebPx Pixel Of oTrans
oTxtEndDEST:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
oGetEndDEST:oFont := TFont():New('Arial',,17*nWebPx,,.T.,,,,.T.,.F.)
...
@ 130*nWebPx,005  Button oConfirma    PROMPT "Confirma" Size 40*nWebPx,15*nWebPx Action fOkProc() Pixel Of oTrans
	cCSSBtN1 :=  "QPushButton{background-image: url(rpo:NGBIOALERTA_02.png);"+cPush+;
		"QPushButton:pressed {background-image: url(rpo:NGBIOALERTA_02.png);"+cPressed+;
		"QPushButton:hover {background-image: url(rpo:NGBIOALERTA_02.png);"+cHover
	oConfirma:SetCSS( cCSSBtN1 )
...
oMark:= MsSelect():New("TMP",,,_aCampos,Nil,Nil,{46*nWebPx,00,127*nWebPx,100*nWebPx})
