...
User Function DOMACW24()
	...
	Define MsDialog oTelaOP Title OemToAnsi("Revenda SENF " + DtoC(dDataBase) ) From 0,0 To 450,302 Pixel of oMainWnd PIXEL

	nLin := 005
	@ nLin,001 Say oTxtEtiq    Var "Num.Etiqueta" Pixel Of oTelaOP
	@ nLin-2,050 MsGet oGetOP  Var _cNumEtqPA Valid VldEtiq() Size 100,10*nWebPx Pixel Of oTelaOP
	oTxtEtiq:oFont := TFont():New('Arial',,12*nWebPx,,.T.,,,,.T.,.F.)
	...
	@ 021*nWebPx,035 Say oTxtProdCod  Var _cProdEmp          Pixel Of oTelaOP
	...
	@ 029*nWebPx,040 say oTxtDescPro  Var _cDescric      Size 075*nWebPx,15*nWebPx Pixel Of oTelaOP
	...
	@ nLin,027 Say oNumOP Var _cNumOP Size 120*nWebPx,10*nWebPx Pixel Of oTelaOP
	...
	oTxtQtdEmp:oFont  := TFont():New('Arial',,12*nWebPx,,.T.,,,,.T.,.F.)

	nLin+= 10*nWebPx
	@ nLin,005 Say oTxtPed Var "Pedido: " Pixel Of oTelaOP
	@ nLin,028 Say oNumPed Var _cNumPed+'-'+_cNomCli Size 120*nWebPx,10*nWebPx Pixel Of oTelaOP
	oTxtPed:oFont:= TFont():New('Arial',,12*nWebPx,,.T.,,,,.T.,.F.)
	oNumPed:oFont:= TFont():New('Arial',,15*nWebPx,,.T.,,,,.T.,.F.)

	nLin+= 05*nWebPx
	@ 070*nWebPx,001 To 145*nWebPx,150 Pixel Of oMainWnd PIXEL

	nLin+= 15*nWebPx
	@ nLin,005   Say   oTxtDtfat Var "Faturamento:" Pixel Of oTelaOp
	@ nLin-1,060 MSGET oDataFat  Var _cDtaFat When .T. VALID vlddata() Size 50*nWebPx,10*nWebPx Pixel Of oTelaOP
	oTxtDtfat:oFont:= TFont():New('Arial',,12*nWebPx,,.T.,,,,.T.,.F.)
	oDataFat:oFont:= TFont():New('Arial',,14*nWebPx,,.T.,,,,.T.,.F.)

	nLin+= 11*nWebPx
	@ nLin,005   Say oTxtSldGrupo Var "Vol.do Pedido:" Pixel Of oTelaOP
	...
	nLin += 15*nWebPx
	@ nLin,005 Say oTxtSaldoEmb  Var "Qtde.por Vol.: " Pixel Of oTelaOP
	...
	nLin += 017*nWebPx
	@ nLin,008 Button oEtiqueta PROMPT "Fechar CAIXA" Size 140,10*nWebPx Action { || FechaSenf(),;
		oTxtProdCod:Refresh(),;
		oTxtProdEmp:Refresh(),;
		oTxtQtdEmp:Refresh(),;
		oNumPed:Refresh(),;
				...
				If SC6->(dbSeek(xFilial("SC2")+XD1->XD1_PVSENF))
					...
					If aScan(aEmbPed,{ |x| x[1] == cGrupoItem }) == 0
						...
						Do While (cAliasSZY)->(!Eof())
							...
							nDisponiveis := Disp2SNF(Alltrim((cAliasSZY)->ZY_PEDIDO + (cAliasSZY)->ZY_ITEM),cNivUsado,(cAliasSZY)->ZY_PEDIDO + (cAliasSZY)->ZY_ITEM)
							nSeparadas   := Separa2SNF((cAliasSZY)->ZY_PEDIDO, (cAliasSZY)->ZY_ITEM,cNivUsado)
						...
						nSaldoGrupo := Separa2SNF(SC6->C6_NUM,'', cNivelFat ,'6')+1
					...
					If _nQtd == nDisponivel
						...
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						oGetQtd:Refresh()
						nPesoBruto := CalPSNF(SC6->C6_NUM,SC6->C6_PRODUTO)
...
Static Function Separa2SNF(cPedido, cItem, cNivelSep, cOcorrencia)
...
Static Function Disp2SNF(cOP,cNivelSep,cPVSENF)
...
Static Function CalPSNF(cPedPeso,cProdPeso)
