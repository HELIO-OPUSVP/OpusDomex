...
INNER JOIN SB1010 SB1 ON SB1.B1_FILIAL = '' AND SB1.B1_COD = SC2.C2_PRODUTO
WHERE SC2.D_E_L_E_T_ <> '*'
AND SC2.C2_FILIAL = '01'
AND SC2.C2_NUM+SC2.C2_ITEM+SC2.C2_SEQUEN BETWEEN '           ' AND 'ZZZZZZZZZZZ'
AND SC2.C2_DATPRF BETWEEN '19500101' AND '20491231'
AND SC2.C2_DATRF = ''
AND SC2.C2_TPOP = 'F'
AND SC2.C2_XXDTPRO BETWEEN '19500101' AND '20491231'
AND SB1.B1_GRUPO BETWEEN 'TRUE' AND 'TRUE'
AND SB1.D_E_L_E_T_ <> '*'

±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
*/
User Function RPCPR01D(lAuto,cImpOP,dDtExped)
	...
	If SC2->( DBSEEK(xFilial()+TMP->C2_NUM,.F.) )
	//RecLock("SC2",.F.)
	//SC2->C2_FLAG  := "S"
	//SC2->( msUnlock() )
	ENDIF
...
Static Function R820Imp(lEnd,wnRel,titulo,tamanho)
	...
	*/
	...
	//cQuery += "		AND SC2.C2_NUM+SC2.C2_ITEM IN ( "
	//cQuery += "			SELECT DISTINCT C2.C2_NUM+C2.C2_ITEM FROM " + RetSqlName("SC2") + " (NOLOCK) C2 "
	//cQuery += "				INNER JOIN " + RetSqlName("SB1") + " (NOLOCK) B1 ON B1.B1_FILIAL = '' AND B1.B1_COD = C2.C2_PRODUTO "
	//cQuery += "			WHERE C2.C2_SEQUEN = '001' "
	//cQuery += "				AND C2.D_E_L_E_T_ <> '*'"
	//cQuery += "				AND B1.D_E_L_E_T_ <> '*'"
	//cQuery += "				AND B1.B1_GRUPO BETWEEN '" + mv_par18 + "' AND '" + mv_par19 + "' "
	//cQuery += "		)	"
	...
	*/
	...
	Else
		...
		While (cAlias)->(!Eof())
			...
			*/
			/*
			If DtoS(SC2->C2_XXDTPRO) < DtoS(mv_par15) .or. DtoS(SC2->C2_XXDTPRO) > DtoS(mv_par16)
			SC2->( dbSkip() )
			Loop
			EndIf
			*/
			/*
			If mv_par17 == 1   // Somente OPs não impressas
			If SC2->C2_FLAG == 'S'
			SC2->( dbSkip() )
			Loop
			EndIf
...
Static Function CabecOp(Tamanho)
	...
	If SC2->C2_NUM+SC2->C2_ITEM+SC2->C2_SEQUEN <> cOPCabec // oprn:npage == 1
		...
		@Li,00 Psay REPLICATE(CHR(151),170)
		Li++         
		@Li,10 PSay "Produto: "+aArray[1][1] Font ofnt2
		...
		( [ cTypeBar ] [ nRow ] [ nCol ] [ cCode ] [ oPr ] [ lCheck ] [ Color ] [ lHort ] [ nWidth ] [ nHeigth ] [ lBanner ] [ cFont ] [ cMode ] [ lPrint ] [ nPFWidth ] [ nPFHeigth ] ) --> Nil
		*/

		_cCODB :=Alltrim(aArray[1][1])
		_cCODB :=StrTran(_cCODB," "," ")  // CARACTER ESPECIAL MLS
		_cCODB :=Alltrim(_cCODB)

		MSBAR2("CODE128",5.90,3.5,_cCODB,opr,.F.,,.T.,0.025,0.5,NIL,NIL,NIL,.F.)

		_cDesc:=StrTran(SB1->B1_DESC,"–"," ")
		...
		@(Li-1),30 Psay SC6->C6_SEUDES + "   Revisao: "+Alltrim(SC6->C6_XXRSTAT)
		//If _nTamOK < _nTamMax //Len(AllTrim(aArray[1][2]))<_nTamMax
		...
		If SC6->(FieldPos("C6_XTAGFOR")) > 0
			If SC6->C6_XTAGFOR == 'S'
				Li+=2
	  			@Li,10 Psay "TAG Fornecedor: SIM" Font ofnt3
			EndIf
		EndIf
