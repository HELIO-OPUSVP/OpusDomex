...
// Criar valida็ใo para nใo deixar alterar o produto/Quantidade quando jแ houver OP gerada para o Item do PV
	...
	If cSeq == '001'  // Apagar somente quando for a OP pai
		...
		cQuery := "SELECT R_E_C_N_O_ FROM " + RetSqlName("SC6") + " (NOLOCK) WHERE C6_FILIAL = '01' AND C6_LOTECTL = '"+cLoteCTL+"' AND D_E_L_E_T_ = '' "
		...
		cQuery := "SELECT C6_NUM, C6_ITEM FROM SC6010 (NOLOCK) WHERE C6_FILIAL = '01' AND C6_NUMOP <> '' AND D_E_L_E_T_ = '' "
		cQuery += "AND C6_NUMOP + C6_ITEMOP NOT IN (SELECT C2_NUM + C2_ITEM FROM SC2010 (NOLOCK) WHERE C2_FILIAL = '01' AND D_E_L_E_T_ = '') "
