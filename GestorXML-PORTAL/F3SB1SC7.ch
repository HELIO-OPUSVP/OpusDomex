...
cNomeJanela := "Seleção de Pedidos - Portal RDT"

hcAlias     := "SC7"

hcCampos    := "C7_NUM,C7_ITEM"
...
/*
If !Empty(aCOLS[N,_nP0])
   cFilIni += " AND B1_BASE = '"+aCOLS[N,_nP0]+"' "
EndIf                                     

If !Empty(aCOLS[N,_nP1])
   cFilIni += " AND B1_XSUBC1 = '"+aCOLS[N,_nP1]+"' "
EndIf                                     

If !Empty(aCOLS[N,_nP2])
   cFilIni += " AND B1_XSUBC2 = '"+aCOLS[N,_nP2]+"' "
EndIf                                     

If !Empty(aCOLS[N,_nP3])
   cFilIni += " AND B1_XSUBC3 = '"+aCOLS[N,_nP3]+"' "
EndIf                                     

*/
cCpoRet  := "C7_NUM"

cCpoPara :=  "C7_NUM" //"C7_PRODUTO"
...
Private cItemPC      := Space(3)
/*
Private cCodProd     := Space(15)
Private cDescProd    := Space(Len(SB1->B1_DESC))
Private cLocal       := Space(2)
Private cGrupo       := Space(4)
Private dNecessidade := CtoD("")
Private cRequisitant := Space(Len(SC1->C1_XENTID))
Private cOrdem       := "Pedido de Compras"
Private lCheck1      := .F.
*/
