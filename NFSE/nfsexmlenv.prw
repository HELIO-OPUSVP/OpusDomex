	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				aadd(aDest, Substring(SA1->A1_EMAIL,1,at(";",SA1->A1_EMAIL)-1 ) )				          
...
//		cString	+= "<codigo>" + allTrim( aProd[1][19] ) + "</codigo>" 	
		cString	+= "<codigo>" + Substring(allTrim( aProd[1][19] ),1,2) + "."+Substring(allTrim( aProd[1][19] ),3,2) + "</codigo>" 		// MAURESI 01/03/2021	
		cString	+= "<aliquota>" + convType( nAliq, 7, 4) + "</aliquota>"	
		cString	+= "</atividade>"
...
// ESPECIFICO DOMEX
	...
	SC5->(DbSeek(xFilial("SC5")+ SD2->D2_PEDIDO)) 
		...
		cLogradPres	:= AllTrim(SC5->C5_ENDPRES )  //Logradouro da prestação
		cNumEndPres	:= AllTrim(SC5->C5_NUMPRES )//Número do logradouro da prestação
		cCompEndPres:= AllTrim(SC5->C5_COMPPRE )//Complemento do logradouro da prestação
		cBairroPres	:= AllTrim(SC5->C5_BAIPRES )//Bairro do logradouro da prestação
		cCepPres	:= AllTrim(SC5->C5_CEPPRES )//Cep da prestação
		cPaisPres	:= AllTrim(SC5->C5_PAIS ) //Pais da prestação
		cMunPrest 	:= AllTrim(SC5->C5_MUNPRES ) //07-Município da prestação do serviço
		cDescMunP 	:= AllTrim(SC5->C5_DESCMUN ) //08-Descrição do Município da prestação do serviço

// FIM ESPECIFICO DOMEX
