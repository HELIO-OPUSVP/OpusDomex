	...
	*/
	...
	If SC7->(DbSeek(xFilial("SC7")+cNumPedido))
		While SC7->C7_NUM == cNumPedido .And. !(Eof())
			...
			cDXOPER:=''
			IF !EMPTY(cXOPER)
				If SX5->( dbSeek( xFilial("SX5") + "ZD" +cXOPER ) )
					cDXOPER   := AllTrim(UPPER(SX5->X5_DESCRI))
					cDescProd :=cDescProd+" - "+cDXOPER
				ENDIF
			ENDIF
...
Static Function RodapePC(nOpc)
	...
	If nOpc == 2
		...
		oPrinter:Say(625,12,"Local de Entrega:" + AllTrim(SM0->M0_ENDENT) + " - " + AllTrim(SM0->M0_BAIRENT) + " - " + AllTrim(SM0->M0_CIDENT) + "/" + AllTrim(SM0->M0_ESTENT) + " - CEP: " + AllTrim(SM0->M0_CEPCOB),oFont9)
		oPrinter:Say(635,12,"Local de Cobranca: " + AllTrim(SM0->M0_ENDCOB) + " - " + AllTrim(SM0->M0_BAIRCOB) + " - " + AllTrim(SM0->M0_CIDCOB) + "/" + AllTrim(SM0->M0_ESTCOB) + " - CEP: " + AllTrim(SM0->M0_CEPCOB),oFont9)
