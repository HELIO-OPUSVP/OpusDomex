...
User Function matr605()        // incluido pelo assistente de conversao do AP5 IDE em 20/07/01
...
Tamanho  := "G"
aOrdem   := {"Numero","Cliente","Produto","Classificação","Nome Cliente Reduzido",} 
cPerg    := "MTR605"
...
±±³07.05.03³MELHOR³Fabrício³Impressao por Classificacao                    ³±±
±±³26.05.04³MELHOR³Fabrício³Impressao por Nome Reduzido do Cliente         ³±±
...
//³ mv_par14            // Da Classificacao ?                         ³   
//³ mv_par15            // Ate Classificacao ?                        ³   
//³ mv_par16            // Do Nome Reduzido do Cliente ?              ³   
//³ mv_par17            // Ate Nome Reduzido do Cliente ?             ³
//³ mv_par19            // Do Vendedor ?                              ³   
//³ mv_par20            // Ate o Vendedor ?                           ³
//³ mv_par21           //  Imprime Margem ?                           ³
//³ mv_par22            // Imprime Orçamento/Projeto ?                ³
//³ mv_par23            // Imprime Orçamento DE R$ ?                  ³
//³ mv_par24            // Imprime Orçamento ATÉ R$ ?                 ³   
...
cCabec1 :="NUMERO IT|CODIGO RAZAO SOCIAL                   |CODIGO        DESCRICAO                      QUANTIDADE PRECO VENDA     TOTAL       PRAZO      DATA     MARGEM   VENDEDOR      CONTATO           DDD TELEFONE      OF"
//Titulo  :=  //   Titulo   + //" (Ordem: " + aOrdem[aReturn[8]]+') - Cliente de "' + MV_PAR16 + '" até "' + MV_PAR17 + '"'
...
Do Case 
	...
	Case aReturn[8] == 6
		dbSetOrder(1)
		dbSeek(xFilial()+MV_PAR19,.T.)
		bCond := {||!Eof() .And. xFilial("SCJ") == SCJ->CJ_NUM .And. ;
					SCJ->CJ_VEND >= MV_PAR19 .And.;
			     	SCJ->CJ_VEND <= MV_PAR20 }
		bQuebra := {|| SCJ->SCJ_NUM }		
EndCase                 
...
/// Fim da inserção	  
	...
	If (  SCK->CK_CLIENTE >= MV_PAR01 .And. SCK->CK_CLIENTE <= MV_PAR02 .And.;
            ...
            SCJ->CJ_CLASSIF >= MV_PAR14 .And. SCJ->CJ_CLASSIF <= MV_PAR15 .And.; 
            SCJ->CJ_VEND    >= MV_PAR19 .And. SCJ->CJ_VEND    <= MV_PAR20 )/* .And.;
...
//            IF SUBSTR(SB1->B1_GRUPO,1,2) == "RF"
        ...
        @ li,092 PSAY SCK->CK_QTDVEN PICTURE TM(SCK->CK_PRCVEN,10,2)
        @ li,103 PSAY xMoeda(SCK->CK_PRCVEN,1,1,SCJ->CJ_EMISSAO)   PICTURE TM(SCK->CK_PRCVEN,11,2)
        @ li,113 PSAY xMoeda(SCK->CK_PRCVEN*SCK->CK_QTDVEN,1,1,SCJ->CJ_EMISSAO)   PICTURE TM(SCK->CK_VALOR ,14,2)
        @ li,131 PSAY SUBSTR(SCK->CK_PRAZO,1,10)
        @ li,142 PSAY SCJ->CJ_EMISSAO
...
//                EndIf
               ...
               //@ li,176 PSAY SUBSTR(SCJ->CJ_CONTATO,1,19)
                @ li,173 PSAY SCJ->CJ_CONTATO
                @ li,196 PSAY SUBSTR(SCJ->CJ_DDD,1,2)
                @ li,199 PSAY SUBSTR(SCJ->CJ_FONE,1,14)
	...
	If ( aQuebra[1] != Eval(bQuebra) .And. aTotal[2,1] > 0 )
      li := li + 1
		...
		if (aReturn[8] == 4 )
						@ li,000 PSAY "Total do Orcamento: "
		EndIf
		@ li,094 PSAY aTotal[2,1]       PICTURE TM(aTotal[2,1],10,2)
		@ li,105 PSAY aTotal[2,2]       PICTURE TM(aTotal[2,2],11,2)
