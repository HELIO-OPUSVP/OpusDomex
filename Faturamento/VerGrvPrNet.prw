...
±±º          ³   Refaz o calculo para PV's que não foi calculado          º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/

User Function VerGrvPrNet(cNumPV)
	Local nItAtu    := 0
	Local nQtdPeso  := 0
	Local nTotalST  := 0
	Local nTotIPI   := 0
...
cQry := " Select  Distinct(C5_NUM) As PEDIDO "
cQry += " From SC5010 SC5 With(Nolock) "
//cQry += " Inner Join SC6010 SC6 With(Nolock) On C6_NUM = C5_NUM And SC6.D_E_L_E_T_ = '' And C6_XPRCNET = 0 "
//cQry += " Where SC5.D_E_L_E_T_ = '' And C5_XPVTIPO = 'OF' And C5_EMISSAO Between '20170101' And '20191231' And C6_DATFAT >= '20200101' "
cQry += " Inner Join SC6010 SC6 With(Nolock) On C6_NUM = C5_NUM And SC6.D_E_L_E_T_ = '' "
cQry += " Where SC5.D_E_L_E_T_ = '' And C5_NUM In ('046123','046120','046127','046128','046125') "
cQry += " Order By C5_NUM "

If Select("XFAT") <> 0
	XFAT->( dbCloseArea() )
EndIf
dbusearea(.t.,"TOPCONN",TCGenQRY(,,cQry),"XFAT",.f.,.t.)
...
While XFAT->(!EOF())
	...
	While ! SC6->(EoF()) .And. SC6->C6_NUM == SC5->C5_NUM
        ...
        nPrecoNet := aValorNet[nOpcao]
        nMargem := ((nPrecoNet - SC6->C6_XCUSUNI) / nPrecoNet) * 100

        //Alert( SC6->C6_NUM +"   Net "+Str(nPrecoNet)+"    Margem "+Str(nMargem))
