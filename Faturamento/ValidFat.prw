...
±±ºPrograma  ³VALIDFAT  ºAutor  ³Helio Ferreira      º Data ³  19/06/19   º±±
...
User Function ValidFat(cPedido,lMsg)
Local _lRet    	:= .T.
...
Local cParaTI		:= "denis.vieira@rdt.com.br;"  // "marco.aurelio@opusvp.com.br;"  
...
Local cParaFAT		:= "priscila.silva@rdt.com.br;juliana.gomes@rdt.com.br;ludmila.guimaraes@rosenbergerdomex.com.br;"        
	...
	If cRisco <> "A"
		...
		If cRisco <> "E"    // Analisa apenas o saldo CR x Limite de Crédito
			...
			If (nValorPVFat + nValorE1) > nLC     .AND.   _lTemDupl   //MAURESI  21/10/2020   adicionado _lTemDupl
				...
				cTexto 	:= "Limite de Crédito insuficiente para a OF "+SC5->C5_NUM+ " - " +  cNomeCli+ " / " +SC5->C5_CLIENTE+"-"+SC5->C5_LOJACLI+ Chr(13)+ Chr(13)
				...
				cTexto += "Enviado email com valores para:" + Alltrim(cParaFIN) + Chr(13)			

          
	//            If Funname() == 'U_MNUCOLETOR'
				If "COLETOR" $ Funname()
					U_MsgColetor(cTexto)  
					//   U_EnvMailto("DOMEX-Valida Msg Analise pelo Coletor- " + Funname(),cTexto,"marco.aurelio@opusvp.com.br",cCC,cArquivo)  
				Else
					If lMsg
						apMsgAlert(cTexto)
					EndIf
			...
			if    _lTemDupl   //MAURESI  11/1/2020   adicionado _lTemDupl		
				...
				Else 
					...
					If nValorPVFat >  (nAdiant + nVlrCartao) 			
						...
						cTexto 	:= " Adiantamento insuficiente para Faturamento da OF "+SC5->C5_NUM+ " - " +  cNomeCli+ ". " + Chr(13)+ Chr(13)
						cTexto 	+= " Valor do Pedido: " + Alltrim(Transform(nValorPVFat,"@E 999,999,999.99")) + Chr(13)
