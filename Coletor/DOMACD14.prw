...
@ 020,075 Say oTxtQtdEmp   Var "Qtd: "      + TransForm(_nQtd,"@E 999,999.9999") Pixel Of oTelaDV
...
IF _lRet == .T.
	...
	Replace TRB->DESCR    With POSICIONE('SB1',1,xFILIAL('SB1')+cCOD,'B1_DESC')
...
*--------------------------------------------------------
USER FUNCTION DEV002LOK(oGetDados)
*--------------------------------------------------------
Local nX      :=0
Local nTOTDEV :=0
			...
			If SBF->( dbSeek( xFilial() + '97' + '97PROCESSO     ' +  SB1->B1_COD + Space(Len(SBF->BF_NUMSERI)) + cLoteCtl) )
				If SBF->BF_QUANT >= oGetDados:aCOLS[nX][02] .and. SB2->B2_QATU >= oGetDados:aCOLS[nX][02]
				 ...
				 //	aadd(_aItem,cLOTE2)                          //Lote Destino   
					...
					ENDIF
				Else
					//U_MsgColetor("Não existe saldo suficiente do produto " + Alltrim(SB1->B1_COD) +" para devolução (saldo = "+Alltrim(Str(SBF->BF_QUANT))+").")
