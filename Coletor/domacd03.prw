...
//-----------------------------------------------------------------------------------------------------------------------------------------------//
// 31/07/13 - Wederson - Tratamento para gravação da quantidade digitada no inventário ,quando divergente,na etiqueta.                           //
//-----------------------------------------------------------------------------------------------------------------------------------------------//
#include "TbiConn.ch"
#include "TbiCode.ch"
#include "Rwmake.ch"
...
if cFilAnt == "01"
	cTitulo:= "Inventário"
ElseIf cFilAnt == '02'
	cTitulo:= "Inventário MG"
Endif

Define MsDialog oInv Title OemToAnsi(cTitulo) From 0,0 To 293,233 Pixel of oMainWnd PIXEL
...
@ 050,045 MsGet oGetQtd  Var _nQtd Valid fOkQtd() Picture "@E 999,999.9999" Size 70,10  Pixel Of oInv
oTxtQtd:oFont := TFont():New('Arial',,17,,.T.,,,,.T.,.F.)
oGetQtd:oFont := TFont():New('Arial',,20,,.T.,,,,.T.,.F.)

@ 130,070 Say oTxtQTDETQ Var Transform(nTxtQTDETQ,"@E 999,999") Pixel Of oInv
oTxtQTDETQ:oFont := TFont():New('Arial',,22,,.T.,,,,.T.,.F.)

@ 130,005 Button "Confirma" Size 40,15 Action fOkProc() Pixel Of oInv
...
For _nI := 1 To Len(_aCols)
	...
	If SB1->( dbSeek(xFilial("SB1")+_aCols[_nI][2]) )
		...
		If lMsErroAuto
			...
			U_MsgColetor("Erro na inclusão do inventário.")
			Exit
		Else
...
If !lMsErroAuto
	...
	For _nI :=1 To Len(_aCols)
		...
		IF _nTotItem == _aCols[_nI][7]  .or. .T.    // Alterado por Hélio para processar o inventario independente da quantidade inventariada bater com o sistema.
			...
			If SB7->(DBSeek(xFilial("SB7")+DTOS(DDATABASE)+_aCols[_nI][2]+_cLocInv+_cEndInv)) 
			...
			If SB7->(DBSeek(xFilial("SB7")+DTOS(DDATABASE)+_aCols[_nI][2]+_cLocInv+_cEndInv))  //lMsErroAuto
				//MostraErro("\UTIL\Inventario\Execucao\")				
				If SB7->B7_STATUS=='2'
					If lPrimeira
						lPrimeira := .F.
						XD1->( dbSetOrder(4))
						XD1->( dbSeek( xFilial("XD1") + Subs(_cProduto,1,15) ) )
			...
			Else //Atualiza a etiqueta , caso a quantidade inventariada seja diferente.
					U_MsgColetor("Erro no processamento do inventário.")
			EndIf
			//U_MsgColetor("Inventário EFETUADO.")
			If _nTotItem == _aCols[_nI][7]
				lInvOK :=.T.
			Else
				lInvOK :=.F.
			EndIf
		ELSE
			//U_MsgColetor("Inventário com quantidades divergente informar a controladoria.")
			lInvOK :=.F.
			SB2->( dbSetOrder(1) )
			If SB2->( dbSeek( xFilial("SB2")+ _aCols[_nI][2]+_cLocInv ) )
				AADD(_aLog,{_aCols[_nI][1],_aCols[_nI][2],_aCols[_nI][3],_aCols[_nI][4],_aCols[_nI][5],_aCols[_nI][6],_aCols[_nI][7],_aCols[_nI][8],SB2->B2_QATU,SB2->B2_VATU1 })
...
IF lInvOK == .T.
	U_MsgColetor("Inventário processado SEM alteração da quantidade total em estoque.")
ELSE
	U_MsgColetor("Inventário processado COM alteração da quantidade total em estoque.")
	//lRetTelaBE :=.F.
ENDIF
...
Define MsDialog oDlgBE Title OemToAnsi("Zerar Marcados") From 0,0 To 293,233 Pixel of oMainWnd PIXEL

@ 005,001 Say oTxtBEMsg Var "O SALDO do endereço será ZERADO. "  Pixel Of oDlgBE
oTxtBEMsg:oFont:= TFont():New('Arial',,15,,.T.,,,,.T.,.F.)

@ 018,001 Say oTxtBEProd Var "Produto "  Pixel Of oDlgBE
@ 018,040 MsGet oGetProdBE Var SB1->B1_COD When .F. Size 70,10  Pixel Of oDlgBE
oTxtBEProd:oFont:= TFont():New('Arial',,15,,.T.,,,,.T.,.F.)
oGetProdBE:oFont:= TFont():New('Arial',,15,,.T.,,,,.T.,.F.)

fGetDadosBE(_aDadosBE)

@ 130,005 Button "Voltar"    Size 40,15 Action FClosebe() Pixel Of oDlgBE
@ 130,070 Button "Processar" Size 40,15 Action fOkBloq()  Pixel Of oDlgBE
	...
	If Marked("MARCA")
		...
		If SB1->(dbSeek(xFilial("SB1")+TRB->PRODUTO))
			...
			If lMsErroAuto
				...
				U_MsgColetor("Erro ao zerar o endereco.(Inclusão SB7)")
			EndIf
			...
			If !lMsErroAuto
				...
				If SB7->(dbSeek(xFilial("SB7")+Dtos(dDataBase)+SB1->B1_COD+TRB->LOC+TRB->ENDERECO))
					...
					EndIf
				Else
					U_MsgColetor("Inventário não encontrado.")
					lMsErroAuto:=.T.
				EndIf
...
lRetTelaBE := .F.  //MLS 28/10/15
//IF lRetTelaBE := .T.
//	U_MsgColetor("Produto deverá ser inventariado em todos os enderecos, ou deverá ser zerado ")
//ENDIF

If lNaoZero
	U_MsgColetor("O produto dever ser inventariado ou zerado em todos os endereços com saldo.")
EndIf
...
//Verifica divergência entre o saldo do endereço com o atual.
SB2->(dbSetOrder(1) )
...
oMark:= MsSelect():New("TMP",,,_aCampos,Nil,Nil,{64,00,127,117})
...
AADD(_aCamposBE,{"ENDERECO"    ,"" ,"Endereço"     ,"@R"              } )
...
If !Empty(_cEtiqueta)
	...
	If XD1->( dbSeek( xFilial("XD1") + _cEtiqueta ) )
		...
		IF alltrim(XD1->XD1_LOCALI) <> alltrim(SUBSTR(_cEndereco,3,LEN(_cEndereco)))
			U_MsgColetor("Endereço etiqueta diferente Endereço coletado. ")
			//RETURN
		ENDIF
		
		If SB1->( dbSeek( xFilial("SB1") + _cProduto) )
			If SB1->B1_LOCALIZ == 'S'
				//Verifica se o produto possui saldo a endereçar.
				//dbSelectArea("SDA")
				SDA->( dbSetOrder(1) )
				...
				If _nSaldo == 0
					...
					If Len(_aCols)>0
						If aScan(_aCols,{ |x| Upper(AllTrim(x[1])) == Trim(_cEtiqueta) }) == 0   //Se a etiqueta não foi utilizada.
							...
							Else
								_lRet:=.F.
								U_MsgColetor("O produto"+Chr(13)+AllTrim(_cProduto)+" não está sendo inventariado neste  momento.")
							EndIf
						Else
							_lRet:=.F.
					...
					Else
						SB7->( dbSetOrder(1) )
						If SB7->( dbSeek(xFilial("SB7") + DtoS(dDataBase) + SB1->B1_COD + XD1->XD1_LOCAL ) )
							nTemp := 0
							While !SB7->( EOF() ) .and. xFilial("SB7") + DtoS(dDataBase) + SB1->B1_COD + XD1->XD1_LOCAL == SB7->B7_FILIAL + DtoS(SB7->B7_DATA) + SB7->B7_COD + SB7->B7_LOCAL
								...
								SB7->( dbSkip() )
							End
							U_MsgColetor("Existem " + Alltrim(Str(nTemp)) + " registros de inventário lançados para este produto nesta data. Entre na rotina de exclusão de inventários para continuar")
							_lRet:=.F.
							U_MsgColetor("Inventário abortado.")
						Else
							SB2->(dbSetOrder(1))
							If SB2->(dbSeek(xFilial("SB2")+SB1->B1_COD+SubStr(_cEndereco,1,2)))
								// Validando se o produto tem divergência de SB2 para SBF
								If QtdComp(SB2->B2_QATU)-QtdComp(SB2->B2_QACLASS) # QtdComp(SaldoSBF(SubStr(_cEndereco,1,2),"",_cProduto))
									U_MsgColetor("Divergência de Saldo Físico X Saldo por Endereço. Não será possível inventariar este produto.")
									_lRet:=.F.
									U_MsgColetor("Inventário abortado.")
								Else
									_cProduto := SB1->B1_COD
...
cMsg += '<h4>Qtd Inventário</font></th>'
...
ELSE
	//U_MsgColetor("Inventário com quantidades divergente informar a controladoria.")
	cMsg += '<font size="3" face="Arial">Inventario NÃO EFETUADO Quantidades Divergente '+'</font>' +CrLf+CrLf
ENDIF
...
cMsg += '<font size="2" face="Arial">Data/Hora do processamento:'+Dtoc(dDataBase)+' às '+Time()+' hr.</font>' +CrLf+CrLf

cMsg += '<font size="3" face="Arial">Rosenberger Domex</font>' +CrLf
cMsg += '<font size="3" face="Arial">Favor não responder. </font>' +CrLf
cMsg += '</body>'+CrLf
cMsg += '</html>'+CrLf
...
If _lAutentica
	If !MailAuth(_cUserAut,_cPassAut)
		U_MsgColetor("Falha na Autenticação do Usuário")
