...
User Function SomaCx2Estrut(cCodEstru,nQtd,cNiv)
...
User Function Separa2Emb(cPedido, cItem, cNivelSep, cOcorrencia,lPeso)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ Disp2Emb   ºAutor  ³Michel Sander     º Data ³  09/29/15   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
User Function Disp2Emb(cOP,cNivelSep,cPVSENF)
...
User Function VerifCx2Disp(cPedSepara,cNivelSep)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ RetEmb2ZW ºAutor  ³Michel Sander       º Data ³  09/29/15   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
User Function RetEmb2ZW(cGrupEmb, nQtEmb, cNivEmb, cCliEmb, cLojEmb)
...
User Function CalPEmb(cPedPeso,cProdPeso)

	LOCAL nOpcPeso := 0

	Define MsDialog oTelaPeso Title OemToAnsi("Peso do VOLUME") From 20,20 To 180,200 Pixel of oMainWnd PIXEL
	nLin := 025
...
Static Function fTelaPeso(cOpTelef,nEmb)
	...
	Define MsDialog oTelaTel Title OemToAnsi("Coleta de Peso") From 20,20 To 260,200 Pixel of oMainWnd PIXEL

	@ 003,003 To 098,088 LABEL " Embalagem " Pixel Of oMainWnd PIXEL
	@ 020,012 Say oTxt1 Var "Informe o Peso da embalagem" Pixel Of oTelaTel
	@ 025,012 Say oTxt2 Var "  para peso de " + If(nEmb==0,"",Alltrim(Str(nEmb))) + " produtos" Pixel Of oTelaTel
	oTxt1:oFont := TFont():New('Arial',,12,,.T.,,,,.T.,.F.)
	oTxt2:oFont := TFont():New('Arial',,12,,.T.,,,,.T.,.F.)
	nLin := 040
	//00005191406x
	//@ nLin,005 To nLin+25,085 LABEL " Peso Líquido " Pixel Of oMainWnd PIXEL
	@  nLin , 005	GROUP oGroup TO nLin+25,085			LABEL " Peso Líquido " OF oMainWnd COLOR CLR_BLUE PIXEL

	nLin += 9
	@ nLin,010 MsGet oGetPeso Var PesoRetorno Valid PesoRetorno > 0 When .T. Picture "@E 999,999,999.99" Size 72,10 Pixel Of oTelaTel
	oGroup:oFont := TFont():New('Arial',,17,,.T.,,,,.T.,.F.)
	oGetPeso:oFont := TFont():New('Arial',,20,,.T.,,,,.T.,.F.)
	nLin+= 050
	@ nLin,028 Button oBotPeso PROMPT "Confirma" Size 35,15 Action {|| nOpcPeso:=1,oTelaTel:End()} Pixel Of oTelaTel
	...
	If cNivelFat == "3" .Or. cTipoSenf == "PR" 	// Embalagem fora da estrutura
		...
		If lSelPorItem
			...
			*/
			...
			Else
				lRotValid := U_DOMETQ87(__mv_par02,__mv_par03,__mv_par04,__mv_par05,cProxNiv,aEmbBip,.F.,nPesoBruto,lColetor,cNumSerie,NIL)  //HUAWEI CRYSTAL
			EndIf
			If !lRotValid
				U_MsgColetor("A etiqueta não será impressa. Verifique os problemas com o  layout da etiqueta e repita a operação.")
				Return ( .F. )
			Else
				...
				Else
					cRotina := "U_DOMETQ87(__mv_par02,__mv_par03,__mv_par04,__mv_par05,cProxNiv,aEmbBip,.T.,nPesoBruto,lColetor,cNumSerie,NIL)" //HUAWEI CRYSTAL
				EndIf
			EndIf
