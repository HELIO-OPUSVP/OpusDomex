...
@ nLin, 060 COMBOBOX oCombo2  VAR cContagem ITEMS aContagem    SIZE 45,10 VALID .T. PIXEL
...
@ 050,045 MsGet oGetQtd  Var _nQtd Valid fOkQtd() Picture "@E 9,999,999.99" Size 70,10  Pixel Of oInv
...
@ 020,005 Say oTxtEtiq   Var "Etiqueta " Pixel Of oInv
@ 020,045 MsGet oGetEtiq Var cEtiqueta  Size 70,10 Valid {||fValEtiq()} Pixel Of oInv
oTxtEtiq:oFont:= TFont():New('Arial',,17,,.T.,,,,.T.,.F.)
oGetEtiq:oFont:= TFont():New('Arial',,20,,.T.,,,,.T.,.F.)
...
@ 130,070 Button oBotao01 Prompt "Sair" Size 40,15 Action Close(oInv) Pixel Of oInv
					...
					If _nSaldoCQ == 0
						If _nSaldoSDA == 0
							...
							Else
								...
								If SZC->( dbSeek( xFilial() + DtoS(dDataBase) + Subs("0"+cEtiqueta,1,12) + ' ' + cContagem ) )
									Reclock("SZC",.F.)
									SZC->ZC_CONTADO += 1
								...
								Else
									...
									SZC->ZC_DATAINV := GetMV("MV_XXDTINV")
...
User Function fOkQtd()
Local _Retorno := .T.

U_uMsgYesNo("Dentro da fOkQtd()")

If _nQtd > 0
	//XD1->( dbSeek( xFilial() + Subs(cEtiqueta,1,13) ) )
	
	If SZC->( dbSeek( xFilial() + DtoS(dDataBase) + "0" + Subs(cEtiqueta,1,11) + " " + cContagem ) )
		Reclock("SZC",.F.)
		SZC->ZC_CONTADO += 1
		If _nQtd <> SZC->ZC_QUANT
			U_MsgColetor("Embalagem inventariada anteriormente com " + Alltrim(Transform(SZC->ZC_QUANT,"@E 999,999,999.99")) + " e alterada agora para " + Alltrim(Transform(_nQtd,"@E 999,999,999.99")) )
...
If SZC->( dbSeek( xFilial() + DtoS(dDataBase) + Subs(cUltEti3,1,13) + cContagem  ) )
	If !Empty(cUltEti3)
		cUltEti3 := cUltEti3 + " - Ok - " + SZC->ZC_USUARIO
	EndIf
EndIf

If SZC->( dbSeek( xFilial() + DtoS(dDataBase) + Subs(cUltEti2,1,13) + cContagem ) )
	If !Empty(cUltEti2)
