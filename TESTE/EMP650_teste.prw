			...
			If SB1->( dbSeek( xFilial() + aCols[x,nPG1_COMP] ) )
				If !Empty(SB1->B1_ALTER) //.or. !Empty(SB1->B1_XXALTER)
					...
					If SB2->( dbSeek( xFilial() + SB1->B1_ALTER ) )
						...
						End
					ENDIF
                    _cDESCCOMP:=POSICIONE('SB1',1,xFILIAL('SB1')+SC2->C2_PRODUTO,"B1_DESC")
                    SB1->( dbSeek( xFilial() + aCols[x,nPG1_COMP] ) )
                    cTexto := "OP "+alltrim(SC2->C2_NUM +'.'+ SC2->C2_ITEM+'.' + SC2->C2_SEQUEN)+"  "+alltrim(SC2->C2_PRODUTO)+" -  "+alltrim(_cDESCCOMP)
                    cTexto +=  Chr(13) + "Produto "+alltrim(_cDESC1) 
                    cTexto +=  Chr(13) + "Qtde "+Alltrim(Transform(aCols[x,nPD4_QUANT],"@E 999,999,999.9999"))
                    cTexto +=  Chr(13) + " "
					cTexto +=  Chr(13) + "O produto   "+ Alltrim(aCols[x,nPG1_COMP]) + " saldo de "+Alltrim(Transform(nSalDisp1,"@E 999,999,999.9999"))
					cTexto +=  Chr(13) + "Alternativo "+ Alltrim(SB1->B1_ALTER)      + " saldo de "+Alltrim(Transform(nSalDisp2,"@E 999,999,999.9999"))
					cTexto +=  Chr(13) + "Deseja utilizar o produto alternativo ? "
					If MsgYesNo( cTexto ,'')
						//Grava log
						RecLock("ZZL",.T.)
