		...
		cContainer := "Container: "+;
			If(EE7->EE7_QTD40H > 0, AllTrim(Str(EE7->EE7_QTD40H)) + "x'40 HC' ","")+ ;
				If(EE7->EE7_QTD20 > 0, AllTrim(Str(EE7->EE7_QTD20)) + "x'20' ","") + ;
					...
					cC2160  := SPACE(60)//EE7->EE7_IMPODE  // GFP - 07/02/2014
					cC2260  := SPACE(60)//EE7->EE7_ENDIMP  // GFP - 07/02/2014
					cC2360  := SPACE(60)//EE7->EE7_END2IM  // GFP - 07/02/2014
					cC2460  := SPACE(60)
					cC2960  := SPACE(60)
					cC3060  := SPACE(60)
					...
					HEADER_P->AVG_C11_60 := EE7->EE7_REFIMP//MLS
					//HEADER_P->AVG_C01100 := EE7->EE7_REFIMP//MLSs

					// TO 
					HEADER_P->AVG_C07_60 := EE7->EE7_IMPODE
					HEADER_P->AVG_C08_60 := EE7->EE7_ENDIMP
					HEADER_P->AVG_C11_20 := SUBSTR(EE7->EE7_ENDIMP,61) //WHRS TE-3348 491818/TVPRXJ(MTRADE-29) - Relatório Proforma Invoice
					HEADER_P->AVG_C09_60 := EE7->EE7_END2IM
					_cOF  :=SUBSTR(EE7->EE7_PEDIDO,15,6)
					HEADER_P->AVG_C07_10 :=ALLTRIM(_cOF)

					//DELIVERY ADDRESS
					HEADER_P->AVG_C06_60 := EE7->EE7_XIMPOD
					HEADER_P->AVG_C13_60 := EE7->EE7_XENDIM
					HEADER_P->AVG_C14_60 := EE7->EE7_XEND2I
					...
					HEADER_P->AVG_C04100 := EECMEND("SA1",1,EE7->EE7_CONSIG+EE7->EE7_COLOJA,.T.,81,1) //WHRS TE-3348 491818/TVPRXJ(MTRADE-29) - Relatório Proforma Invoice
					...
					HEADER_P->AVG_C04_20 := AllTrim(Transf(EE7->EE7_PESBRU,"9,999.99"))
					...
					HEADER_P->AVG_C03_30 := EE7->EE7_XXINCO//--DESC INCOTERM

					_cCONTATO:=POSICIONE('SA1',1,xFILIAL('SA1')+EE7->EE7_IMPORT,'A1_CONTATO')
					IF !EMPTY(_cCONTATO)
						_cCONTATO:="Contact :"+_cCONTATO
						HEADER_P->AVG_C12_60 := _cCONTATO//--CONTATO
					ENDIF
					HEADER_P->AVG_C20_60 :=EE7->EE7_XCONT2
					...
					If !lNacional
						...
						IF AT(EasyGParam("MV_AVG0037",,"INGLES"), EE7->EE7_IDIOMA) >0
							If AT("ORIGIN",cDescrGen)=0
								If empty(MemoLine(cDescrGen,90,1))
									cDescrGen := cDescrGen + ENTER +"ORIGIN: BRAZIL. EXCEPT XXX, ORIGEM: XXXX." //"ORIGIN: BRAZIL. EXCEPT XXX, ORIGEM: XXXX."
						...
						ElseIf AT("ESPANHOL", EE7->EE7_IDIOMA) >0
							If AT("ORIGEN",cDescrGen)=0
								If nItens == 1
									If empty(MemoLine(cDescrGen,90,1))
										cDescrGen := cDescrGen + ENTER +"ORIGEN: BRASIL. EXCEPTO EL PRODUCTO XXXX , ORIGEM XXX  ." //"ORIGEN: BRASIL. EXCEPTO EL PRODUCTO XXXX , ORIGEM XXX  ."
									ElseIf empty(MemoLine(cDescrGen,90,2))
										cDescrGen := cDescrGen + ENTER +"ORIGEN: BRASIL. EXCEPTO EL PRODUCTO XXXX , ORIGEM XXX  ." //"ORIGEN: BRASIL. EXCEPTO EL PRODUCTO XXXX , ORIGEM XXX  ."
									ElseIf empty(MemoLine(cDescrGen,90,3))
										cDescrGen := cDescrGen + ENTER +"ORIGEN: BRASIL. EXCEPTO EL PRODUCTO XXXX , ORIGEM XXX  ." //"ORIGEN: BRASIL. EXCEPTO EL PRODUCTO XXXX , ORIGEM XXX  ."
								...
								Else
									If empty(MemoLine(cDescrGen,90,1))
										cDescrGen := cDescrGen + ENTER +"ORIGEN: BRASIL. EXCEPTO LOS PRODUCTOS XXXX , ORIGEM XXX  ." //"ORIGEN: BRASIL. EXCEPTO LOS PRODUCTOS XXXX , ORIGEM XXX  ."
									ElseIf empty(MemoLine(cDescrGen,90,2))
										cDescrGen := cDescrGen + ENTER +"ORIGEN: BRASIL. EXCEPTO LOS PRODUCTOS XXXX , ORIGEM XXX  ." //"ORIGEN: BRASIL. EXCEPTO LOS PRODUCTOS XXXX , ORIGEM XXX  ."
									ElseIf empty(MemoLine(cDescrGen,90,3))
										cDescrGen := cDescrGen + ENTER +"ORIGEN: BRASIL. EXCEPTO LOS PRODUCTOS XXXX , ORIGEM XXX  ." //"ORIGEN: BRASIL. EXCEPTO LOS PRODUCTOS XXXX , ORIGEM XXX  ."
					...
					//HEADER_P->AVG_C02_30 := IF(Left(SYQ->YQ_COD_DI,1) == "4",IF(lIngles,"BY AIR","AEREO"),If(SYQ->YQ_DESCR)) // VIA
					//NCF - 11/06/2013 - Adicionar descrição da Via quando Marítimo(avaliar cadastro de idiomas para descrição da via)
					HEADER_P->AVG_C02_30 := PEM49TRANSPOR()
					...
					HEADER_P->AVG_C07_30 := SubStr(aNotify[2],61) //WHRS TE-3348 491818/TVPRXJ(MTRADE-29) - Relatório Proforma Invoice
					...
					SD2->(DbSeek(xFilial("SD2")+EE7->EE7_PEDFAT))
					IF SD2->(FOUND())
						//HEADER_P->AVG_C25_60 := SD2->D2_DOC+' - '+SUBSTR(DTOC(SD2->D2_EMISSAO),7,2)+'/'+SUBSTR(DTOC(SD2->D2_EMISSAO),5,2)+'/'+SUBSTR(DTOC(SD2->D2_EMISSAO),1,4)
						//HEADER_P->AVG_C25_60 := SD2->D2_DOC+' - '+DTOC(SD2->D2_EMISSAO)
						HEADER_P->AVG_C25_60 := DTOC(SD2->D2_EMISSAO)
					ENDIF
					IF EE7->EE7_DIASPA >0
						HEADER_P->AVG_C10_60 :='net '+cValtoChar(EE7->EE7_DIASPA)+' days '
					ENDIF
					//
					// L/C
					...
					IF ( !EMPTY(BuscaInst(EE7->EE7_PEDIDO,OC_PE,BC_DBR)))
						...
						If !Empty(EEJ->EEJ_FAVORE)
							HEADER_P->AVG_C14_60 := "ACCOUT:  "+AllTrim(EEJ->EEJ_NUMCON) //"ACCOUT:  "
							HEADER_P->AVG_C21_60 := "BENEFICIARY:  "+ALLTRIM(cEXP_NOME) //"BENEFICIARY:  "

							HEADER_P->AVG_C05_60 := Alltrim(EEJ->EEJ_FAVORE)
							IF AT(EasyGParam("MV_AVG0037",,"INGLES"), EE7->EE7_IDIOMA) >0
								HEADER_P->AVG_C14_60 := "BENEFICIARY:  "+ALLTRIM(cEXP_NOME) //"BENEFICIARY:  "

								If !Empty(EEJ->EEJ_BENEDE)
									HEADER_P->AVG_C21_60 := "ACCOUT:  "+ALLTRIM(EEJ->EEJ_BENEDE)  //"ACCOUT:  "
					...
					HEADER_H->(dbAppend())             // GFP - 13/11/2014
					AvReplace("HEADER_P","HEADER_H")   // GFP - 13/11/2014
					HEADER_H->(dbUnlock())             // GFP - 13/11/2014
				...
				Endif
				IF Select("Work") > 0
					Work->(E_EraseArq(cFile,cFile+TEOrdBagExt()))
				EndIf
				RestOrd(aOrd)
				EE8->(DBSETORDER(1))
				EE3->(DBSETORDER(1))
...
*-------------------------*
	...
	While EE8->(!Eof() .And. EE8_FILIAL == xFilial("EE8")) .And.;
		...
		While EE8->(!Eof() .And. EE8_FILIAL == xFilial("EE8")) .And.;
			...
			AppendDet()
			_cdesc := posicione('SB1',1,xFILIAL('SB1')+EE8->EE8_COD_I,'B1_DESC')
			DETAIL_P->AVG_C01_20 := Transf(EE8->EE8_SLDINI,cPictQtde)
			DETAIL_P->AVG_C02_20 := Transf(EE8->EE8_COD_I,AVSX3("EE8_COD_I",6))
			DETAIL_P->AVG_C02_20 := SUBSTR(DETAIL_P->AVG_C02_20,0,15)  // LRS 29/10/2013- Substr cortando string no 4° Caracter, foi tirar o parametro 4, para 0
			DETAIL_P->AVG_C03_20 := Transf(EE8->EE8_POSIPI,AVSX3("EE8_POSIPI",6))
			DETAIL_P->AVG_C04_20 := AllTrim(Transf(EE8->EE8_SLDINI,cPict))
			DETAIL_P->AVG_C05_60 := Transf(_cdesc,AVSX3("B1_DESC",6))
			DETAIL_P->AVG_C02_60 := Transf(EE8->EE8_PART_N,AVSX3("EE8_PART_N",6))
			DETAIL_P->AVG_C01_10 := posicione('SB1',1,xFILIAL('SB1')+EE8->EE8_COD_I,'B1_UM')
			...
			DETAIL_P->AVG_C07_20 := AllTrim(Transf(EE8->EE8_PRCTOT,cPict))

			nTotQtde := nTotQtde+EE8->EE8_SLDINI
			nTotal   := nTotal  +EE8->EE8_PRCTOT //EE8->EE8_PRCINC
			...
			//THTS - Trata a Descricao do produto
			//cMemo := MSMM(EE8->EE8_DESC)
			//cMemo := StrTran(cMemo, "®", CHR(169))

			//For i := 1 To MlCount(cMemo,35)
			//	cMemoTamRel := Alltrim(MemoLine(cMemo,35,i))
			//	IF !EMPTY(cMemoTamRel)
			//DETAIL_P->AVG_C01_60 := cMemoTamRel
			//UnlockDet()
			//AppendDet()
			//	ENDIF
			//Next
...
Static Function GetEmbs()
	...
	While EE8->(!EOF()) .and. EE8->(EE8_FILIAL + EE8_PEDIDO) == xFilial("EE8") + EE7->EE7_PEDIDO
		If (nPos := aScan(aEmbs,{|X| AllTrim(Upper(X)) == AllTrim(Upper(EE8->EE8_EMBAL1)) })) == 0
