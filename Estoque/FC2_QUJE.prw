...
User Function FC2_QUJE(cOP)
	...
	cQuery += "ISNULL((SELECT SUM(D3_QUANT) FROM "+RetSqlName("SD3")+" WHERE D3_FILIAL = C2_FILIAL AND D3_OP = C2_NUM+C2_ITEM+C2_SEQUEN "
	cQuery += "AND D3_TM = '010' AND D3_ESTORNO = '' AND D_E_L_E_T_=''),0) AS D3_APTO "
	cQuery += ",(SELECT MAX(D3_EMISSAO) FROM SD3010 WHERE D3_FILIAL = C2_FILIAL AND D3_OP = C2_NUM+C2_ITEM+C2_SEQUEN AND D3_TM = '010' AND D3_ESTORNO = '' AND D_E_L_E_T_='' ) AS D3EMISSAO "
