...
±±ºPrograma  ³ DOMETQ02 ºAutor  ³ Felipe A. Melo     º Data ³  08/12/2013 º±±
...
//Private cSeqEtiq := AllTrim(GetMV("MV_XXSEQET"))
Private cRotacao := "R" //(N,R,I,B)

Private mv_par01:= Space(9)  //Numero Senf
Private mv_par02:= 0         //Qtd Embalagem
Private mv_par03:= 0         //Qtd Etiquetas
...
aAdd(aPar,{1,"Senf + Item"       ,   Space(9) ,"@!"      ,/*"NaoVazio()"*/,      ,,60,.T.})
...
//Localiza SC5
SC5->(DbSetOrder(1))
If lAchou .And. SC5->(!DbSeek(xFilial("SC5")+Subs(mv_par01,1,6) ))
	Alert("Numero de Senf "+Subs(mv_par01,1,6)+" não localizado!")
	lAchou := .F.
EndIf

//Localiza SC6
SC6->(DbSetOrder(1))
If lAchou .And. SC6->(!DbSeek(xFilial("SC6")+Subs(mv_par01,1,8) ))
	Alert("Item Senf "+Subs(mv_par01,7,2) +" não localizado!")
	lAchou := .F.
EndIf
...
If lAchou .And. ("HUAWEI" $ Upper(SA1->A1_NOME))
	EtLay002()
Else
   Alert("Etiqueta de PA desenvolvida apenas para o cliente Huawei.")
EndIf

Return lAchou

//*************************************************************************************
Static Function EtLay002()   // Layout de impressão de etiqueta 001 - Huawai
...
If lAchou
	...
	Do Case
		...
		OtherWise
			cCdAnat1 := " E R R O "
			cCdAnat2 := " E R R O "
			Alert("Erro no tratamento dos codigos ANATEL (DOMETQ02)."+Chr(13)+Chr(10)+"Favor procurar TI e informar dificuldade! "+Chr(13)+Chr(10)+"OP:"+mv_par01)
			lAchou   := .F.
...
For x:=1 To mv_par03
   ...
   cSeqEtiq := NEXTSEQ()	
	...
	/*15*/  MSCBSAYBAR(nCol+(nLin*09),nIni+94+10 ,"Quantity: "+StrZero(mv_par02,3)                             ,cRotacao,"MB07",8.361,.F.,.F.,.F.,,3,2,.F.,.F.,"1",.T.)
