		...
		Else
			...
			If Empty(cNumOpBip)
				...
				If lValidaRot
					...
					cQryPkl := " SELECT XD1_FILIAL FILIAL,LEFT(XD1.XD1_OP,8) OP,COUNT(*) QTDETQ,ISNULL(TMP1.QTD7,0) QTD7,ISNULL(TMP2.QTD8,0) QTD8,ISNULL(TMP3.QTD6,0) QTD6 "
					cQryPkl += ENTER + " FROM " + RetSqlName("XD1") + " XD1 (NOLOCK) "
					cQryPkl += ENTER + " LEFT JOIN ( "
					cQryPkl += ENTER + " SELECT XD1_FILIAL FIL,LEFT(XD1_OP,8) OP,COUNT(*) QTD7 FROM " + RetSQlName("XD1") +"  XD11 (NOLOCK) "
					...
					cQryPkl += ENTER + " WHERE XD11.D_E_L_E_T_ ='' AND XD11.XD1_FILIAL <> '' AND XD11.XD1_OCORRE = '7' GROUP BY XD11.XD1_FILIAL,LEFT(XD11.XD1_OP,8) "
					cQryPkl += ENTER + " ) TMP1 ON TMP1.FIL= XD1.XD1_FILIAL AND TMP1.OP = LEFT(XD1.XD1_OP,8)  "
					cQryPkl += ENTER + " LEFT JOIN ( "
					cQryPkl += ENTER + " SELECT XD1_FILIAL FIL,LEFT(XD1_OP,8) OP,COUNT(*) QTD8 FROM " + RetSqlName("XD1") + " XD11 (NOLOCK) "
					cQryPkl += ENTER + " JOIN " + RetSqlName("SB1") + " SB1 (NOLOCK) ON SB1.D_E_L_E_T_ = '' AND SB1.B1_FILIAL = '' AND SB1.B1_COD = XD11.XD1_COD AND B1_TIPO <> 'ME' AND B1_GRUPO <> 'DIV' "
					cQryPkl += ENTER + " WHERE XD11.D_E_L_E_T_ ='' AND XD11.XD1_FILIAL <> '' AND XD11.XD1_OCORRE = '8' GROUP BY XD11.XD1_FILIAL,LEFT(XD11.XD1_OP,8) "
					cQryPkl += ENTER + " ) TMP2 ON TMP2.FIL= XD1.XD1_FILIAL AND TMP2.OP = LEFT(XD1.XD1_OP,8) "
					cQryPkl += ENTER + " LEFT JOIN ( "
					cQryPkl += ENTER + " SELECT XD1_FILIAL FIL,LEFT(XD1_OP,8) OP,COUNT(*) QTD6 FROM " + RetSqlName("XD1") + " XD11 (NOLOCK) "
					...
					cQryPkl += ENTER + " WHERE XD11.D_E_L_E_T_ ='' AND XD11.XD1_FILIAL <> '' AND XD11.XD1_OCORRE = '6' GROUP BY XD11.XD1_FILIAL,LEFT(XD11.XD1_OP,8) "
					cQryPkl += ENTER + " ) TMP3 ON TMP3.FIL= XD1.XD1_FILIAL AND TMP3.OP = LEFT(XD1.XD1_OP,8)  "
					cQryPkl += ENTER + " WHERE XD1.D_E_L_E_T_ ='' "
					cQryPkl += ENTER + " AND LEFT(XD1.XD1_OP,8) = '" + SC2->C2_NUM + SC2->C2_ITEM  + "' "
					cQryPkl += ENTER + " AND XD1_OCORRE   IN ( '8','7','6') "
					cQryPkl += ENTER + " GROUP BY XD1.XD1_FILIAL,LEFT(XD1.XD1_OP,8),TMP1.QTD7,TMP2.QTD8,TMP3.QTD6 "
					If Select("TPKL") > 0
						TPKL->(DBCloseArea())
					EndIf
				...
				If SB1->B1_XKITPIG <> "S" .or. !GetMv("MV_XVERKIT")
					...
					If lOkFlex
						If Alltrim(cVerUsoGr) $ "CORD/0007/JUMP/FLEX/CMTP"  // PCON retirado em 04/11/21 por Helio/Ricardo
							...
							cProxEmb   := Posicione("SB1",1,xFilial("SB1")+aRetEmbala[1],"B1_DESC")
							nQProxEmb  := Round(aRetEmbala[2],0)
							oQProxEmb:Refresh()
							oProxEmb:Refresh()
			...
			If SB1->B1_XKITPIG <> "S" .or. !GetMv("MV_XVERKIT")
				...
				ElseIf AllTrim(_cGrupoUso) $ "CORD/0007/JUMP/CMTP" .Or. LEFT(AllTrim(_cGrupoUso),3) == "DIO"  // PCON retirado em 04/11/21 por Helio/Ricardo
					...
					If "HUAWEI DO BRASIL" $ Upper(SA1->A1_NOME)
						U_DOMETQ98(cNumOpBip,NIL,1,1,"1",aUsoSerie,.T.,0,.F., Alltrim(cEtiqOrig),,'OP:'+SC2->C2_NUM+SC2->C2_ITEM,cCodHuawei) //Layout 98 - Etiqueta Somente com CODBAR
					Else
						If nRadio == 1 // Embalagem SEM Etiqueta de Fornecedor
				...
				Else
					...
					If SubStr(AllTrim(_cGrupoUso),1,3) == "TRU" .OR. SubStr(AllTrim(_cGrupoUso),1,3) == "CMT" .OR. SubStr(AllTrim(_cGrupoUso),1,3) == "FTT"
						If lEricsson .And.  SubStr(AllTrim(_cGrupoUso),1,3) == "FTT"
						...
						Else
							...
							If AllTrim(_cGrupoUso) == "JUMP" .Or. SubStr(AllTrim(_cGrupoUso),1,3)=="TRU" .Or. AllTrim(_cGrupoUso) == "FLEX" .Or. AllTrim(_cGrupoUso) == "CMTP" .Or. AllTrim(_cGrupoUso) == "FTTA"
								...
								If (SubStr(AllTrim(_cGrupoUso),1,3)=="TRU" .Or. AllTrim(_cGrupoUso) == "FLEX" .Or. AllTrim(_cGrupoUso) == "FTTA" )  .And. lEricsson
									...
									U_DOMETQ98(SC2->C2_NUM+SC2->C2_ITEM+SC2->C2_SEQUEN,NIL,nQEmbAtu,1,"1",aSerial,.T.,0,lUsaColet, "","","OP: " + SC2->C2_NUM+SC2->C2_ITEM+SC2->C2_SEQUEN) //Layout 98 - Etiqueta Somente com CODBAR
								Else
			...
			Else
				...
				If nQtdBip == (SC2->C2_QUANT - SC2->C2_QUJE) .Or.  (nQtTotN1 == SC2->C2_QUANT) .Or. ((nQtTotN1 + nQtdBip) == SC2->C2_QUANT)  // Trocado de C2_XXQUJE para C2_QUJE      por Hélio em 25/09/18
					...
					Else
						...
						U_DOMETQ98(SC2->C2_NUM+SC2->C2_ITEM+SC2->C2_SEQUEN,NIL,nQtdBip,1,"1",aSerial,.T.,0,lUsaColet, "","","OP: " + SC2->C2_NUM+SC2->C2_ITEM+SC2->C2_SEQUEN) //Layout 98 - Etiqueta Somente com CODBAR
					EndIf
					aSerial := {}
					aQtdBip  := {}
					aQtdEtiq := {}
			...
			If lApontaOP
				...
				Else
					...
					If lEricsson
						...
						Endif
						MsgRun("Imprimindo etiqueta Layout 41   2/2","Aguarde...",{|| U_DOMETQ41(__mv_par02,__mv_par03,__mv_par04,__mv_par05,cProxNiv,aQtdBip,.T.,nPesoBip,lColetor,cNumSerie,Nil,"000000"     ) }) // Layout 41
					Endif
	...
	If __mv_par06 == "98"
		...
		If lApontaOP
			If !U_ETQMTA250("010",SC2->C2_NUM+SC2->C2_ITEM+SC2->C2_SEQUEN,SC2->C2_PRODUTO,SC2->C2_LOCAL,nQtdKit,"P",cNumPeca)
				...
				Return
			Else
				MsgRun("Imprimindo etiqueta Layout 98","Aguarde...",{|| lRetEtq := U_DOMETQ98(SC2->C2_NUM+SC2->C2_ITEM+SC2->C2_SEQUEN,NIL,nQEmbAtu,1,"1",aQtdBip,.T.,0,lUsaColet, "",cNumPeca,"OP"+SC2->C2_NUM+SC2->C2_ITEM+SC2->C2_SEQUEN)  }) //Layout 98 - Etiqueta Somente com CODBAR
				Sleep(5000)		// Delay de 5 segundos para buffer
				cDomEtDl36_CancLay := __mv_par06 // Salva a impressao atual	para possível cancelamento
			EndIF
		EndIf
	...
	Else
		...
		If cDomEtDl36_CancLay == "94"
			...
			Else
				...
				U_DOMETQ98(cDomEtDl32_CancOP,cDomEtDl33_CancEmb,cDomEtDl34_CancKit,cDomEtDl35_CancUni,cDomEtDl38_CancNiv,aDomEtDl3A_CancFil,.T.,cDomEtDl39_CancPes,lColetor,cNumSerie,,"OP: " + cDomEtDl32_CancOP) //Layout 98 - Etiqueta Somente com CODBAR
			EndIf
		EndIf
		...
		if U_VALIDACAO("RODA")
			IF lUltVtal
				SA1->(dbSeek(xFilial("SA1")+SC2->C2_CLIENT))
				if fVTal(SA1->A1_COD, SA1->A1_LOJA)
					U_DOMETQ49(cOiDl32_CancOP, cOiDl33_CancPro, cOiDl34_CancPed, cOiDl35_CancUni, cOiDl38_CancDat, .F. , "" , 0, cOiDl39_CancQtd,cFila)
				Endif
			Endif
		Endif
...
Static Function fImpSeri(cOP,cNumSerie,aFilhas)
	...
	If SC2->(dbSeek(xFilial("SC2")+cOP))
		...
		ElseIf cLayout == "41"
			U_DOMETQ98(cOp,Nil,1,1,'1',aFilhas,.T.,_PesoAuto,lColetor, cNumSerie,,"OP: " + cOp) //Layout 98 - Etiqueta Somente com CODBAR
			If lEricsson
