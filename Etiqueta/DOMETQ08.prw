...
±±ºPrograma  ³ DOMETQ08 ºAutor  ³ Michel A. Sander   º Data ³  08/05/2014 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Etiqueta Modelo 06                                         º±±
±±º          ³                                                            º±±
...
User Function DOMETQ08(cNumOP,cNumSenf,nQtdEmb,nQtdEtq,cNivel,aFilhas,lImpressao,nPesoVol,lColetor,cNumSerie,cNumPeca)
...
If lAchou
	...
	Do Case
		...
		OtherWise
			...
			cCdAnat2 := " E R R O "
			If !Empty(cNumOP)
				Alert("Erro no tratamento dos codigos ANATEL (DOMETQ08)."+Chr(13)+Chr(10)+"Favor procurar TI e informar dificuldade! "+Chr(13)+Chr(10)+"OP:"+cNumOP)
			ElseIf !Empty(cNumSenf)
				Alert("Erro no tratamento dos codigos ANATEL (DOMETQ08)."+Chr(13)+Chr(10)+"Favor procurar TI e informar dificuldade! "+Chr(13)+Chr(10)+"Senf:"+cNumSenf)
			EndIf
			lAchou   := .F.
...
For x := 1 To mv_par03
	...
	EndIf
	/*01*/   MSCBSAY(nCol,nIni+(nLin*13)   ,"TESTES:"                                                                                 ,cRotacao,"0","30,30")
	/*01*/   MSCBSAY(nCol,nIni+(nLin*14)   ,"SM - IL<0.30dB   RL:>45dB (PC)   >55dB (APC)"                                            ,cRotacao,"0","30,30")
	/*01*/   MSCBSAY(nCol,nIni+(nLin*15)   ,"SM PREMIUM - IL:<0.1dB   RL:>45dB (PC) >55dB (APC)"                                      ,cRotacao,"0","30,30")
	/*01*/   MSCBSAY(nCol,nIni+(nLin*16)   ,"MM - IL:<0.30dB  RL:>30dB"									                                     ,cRotacao,"0","30,30")
	...
	/*01*/   MSCBSAY(nCol+90,nIni+(nLin*19),"Layout 006"                                                                              ,cRotacao,"0","30,30")
