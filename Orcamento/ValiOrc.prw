...
±±ºPrograma  ³VALIORC  ºAutor  ³Helio Ferreira      º Data ³  19/06/19    º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Valida se um orçamento pela analise de crédto do cliente   º±±
±±º          ³                                                            º±±
...
//#################################
			If cBloq == "1"   //MAURESI  25/03/2021   Testa Cliente Bloqueado
				...
				cAssunto := "Analise de Credito - CLIENTE BLOQUEADO - Orçamento: " +M->CJ_NUM+ " - " +  cNomeCli 
...
//##############################
	If cRisco <> "A"
		...
		If cRisco <> "E"    // Analisa apenas o saldo CR x Limite de Crédito
			...
			// VALOR DO PEDIDO QUE ESTÁ SENDO FATURADO
            nValorPVFat := 0
            _lTemDupl := .F.
			...
			If (nValorPVFat + nValorE1) > nLC     .AND.   _lTemDupl   //MAURESI  21/10/2020   adicionado _lTemDupl
				...
				// Envia WorkFlow  
				cNomeCli := Posicione("SA1",1,xFilial("SA1")+M->CJ_CLIENTE+M->CJ_LOJA,"A1_NREDUZ")
				cAssunto := "Analise de Credito - LIMITE Insuficiente - Orçamento: " +M->CJ_NUM+ " - " +  cNomeCli 
				cTexto 	:= "Limite de Crédito insuficiente para o Orçamento "+M->CJ_NUM+Chr(13)
				...
				cTexto 	+= "Limite de Crédito: " + Alltrim(Transform(nLC,"@E 999,999,999.99")) + Chr(13)     
				//cTexto   := StrTran(cTexto,Chr(13),"<br>")
			...
			//	cTexto += "Este pedido será bloqueado para faturamento" + Chr(13)			    
		...
		Else
         ...
         // RISCO E
			...
			if    _lTemDupl   //MAURESI  11/1/2020   adicionado _lTemDupl		
				...
				//_____________________________________
				_lAdtIntl := iif(M->CJ_CONDPAG $  GetMV("MV_XADTINT")  ,.T.,.F.)  //Condiçao de Adiantamento Internacional
				if _lAdtIntl 
					// Se for ADT Internacional, não executa Validacoes    

						cNomeCli := Posicione("SA1",1,xFilial("SA1")+M->CJ_CLIENTE+M->CJ_LOJA,"A1_NREDUZ")
						cAssunto := "Analise de Credito - ADIANTAMENTO INTERNACIONAL - Orçamento: " +M->CJ_NUM+ " - " +  cNomeCli 
						cTexto := " ## ATENCAO ##  " + Chr(13)+ Chr(13)   
						cTexto += " Condição de Pagamento de Adiantamento INTERNACIONAL utilizada no Orçamento "+M->CJ_NUM+ " - " +  cNomeCli+ " / " +M->CJ_CLIENTE+"-"+M->CJ_LOJA+ Chr(13)+ Chr(13)
				...
				Else 
					...
					If nValorPVFat >  (nAdiant + nVlrCartao) 			
						//³Envia WorkFlow
						cNomeCli := Posicione("SA1",1,xFilial("SA1")+M->CJ_CLIENTE+M->CJ_LOJA,"A1_NREDUZ")
						cAssunto := "Analise de Credito - ADIANTAMENTO Insuficiente - Orçamento: " +M->CJ_NUM+ " - " +  cNomeCli+ " / " +M->CJ_CLIENTE+"-"+M->CJ_LOJA+ Chr(13)+ Chr(13)
						cTexto 	:= " Adiantamento insuficiente para inclusão do Orçamento "+M->CJ_NUM +Chr(13)
						...
						cTexto 	+= " RA + Cartão Crédito: " + Alltrim(Transform((nAdiant+nVlrCartao),"@E 999,999,999.99"))  + Chr(13)  
						//cTexto   := StrTran(cTexto,Chr(13),"<br>")
