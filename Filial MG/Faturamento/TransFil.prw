...
±±ºPrograma  ³ReSldEst  ºAutor  ³Osmar Ferreira      º Data ³  05/03/21   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Relatório de Saldos em estoque por armazém                 º±±
±±º          ³ para transferencia para a filial MG                        º±±
...
User Function TransFil()

	Local oReport
	Private _cPerg      := "TRAFIL"
...
Static Function ReportDef()
	...
	oReport:=TReport():New("TransFil","Material a ser transferido para a Filial. " ,"TransFil",{|oReport| PrintReport(oReport)},"Armazém "+cLocTransf)
...
Static Function xQuery1
	...
	Select	B2_FILIAL, B2_COD As 'PRODUTO', B2_QATU As 'SALDO_ATUAL', '' As 'CODCLI', 
			'' As 'ITEM_CLI', '' As 'SHIPHUAWEI','' As 'DATA_CLI', '99' As 'TP_OPER_RDT'
    From %Table:SB2% SB2 (Nolock)
    Inner Join %Table:SB1% SB1 (Nolock) On SB1.%NotDel% And B1_COD = B2_COD 
    Where SB2.%NotDel% And B2_FILIAL = '01' And B2_QATU > 0 And
