	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			If MsSeek(xFilial("SF3")+cClieFor+cLoja+cNota+cSerie)
				...
				//----------------------------------------------------------------------------------------------------
				//ï¿½DOMEX - Adiciona Mensagem Padrao do Produto nas Msg da NF  - Marco Aurelio  01/12/2014 - Chamado 001420
				//----------------------------------------------------------------------------------------------------
				If !Empty(SB1->B1_MENPAD) .And. !AllTrim(FORMULA(SB1->B1_MENPAD)) $ cMensCli
					If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
						cMensCli += " "
					EndIf
					cMensCli += AllTrim(FORMULA(SB1->B1_MENPAD))
				EndIf
				//----------------------------------------------------------------------------------------------------
			...
			Else
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
					   ...
					   If !Empty(SC5->C5_XMSGTES) .And. !AllTrim(SC5->C5_XMSGTES) $ cMensFis
						...
						EndIf
						*/
						// MAURESI - 08/02/2021  -  INIBIDO TRECHO ACIMA E HABILITADO ApedCom ABAIXO
						        	...
						        	If SB1->B1_XXPROIN == "S"
						            	_cPPB := ""
								      	...
								      	If !Empty(SC6->C6_SEUCOD)
								        	If Empty(_cPPB)
									        	aProd[Len(aProd)][04] := Alltrim(cAuxPPB)+"|"+AllTrim(SC6->C6_SEUCOD)	// Descriï¿½ï¿½o Detalhada
									        	aProd[Len(aProd)][25] := ""                                             // Informaï¿½ï¿½es Adicionais
									      	Else
									        	aProd[Len(aProd)][04] := _cPPB             // Descriï¿½ï¿½o do PPB
									        	aProd[Len(aProd)][25] := Alltrim(cAuxPPB)+"|"+AllTrim(SC6->C6_SEUCOD) // Informaï¿½ï¿½es Adicionais
									      	EndIf   
								      	EndIf
										If !Empty(SC6->C6_SEUDES)
											If Empty(_cPPB)   
												aProd[Len(aProd)][04] := AllTrim(cAuxPPB)+"|"+AllTrim(SC6->C6_SEUCOD)+"-ITEM "+AllTrim(SC6->C6_SEUDES)	// Descriï¿½ï¿½o Detalhada
												aProd[Len(aProd)][25] := ""                                                // Informaï¿½ï¿½es Adicionais
											Else
												aProd[Len(aProd)][04] := _cPPB																// Descriï¿½ï¿½o do PPB
												aProd[Len(aProd)][25] := AllTrim(cAuxPPB)+"|"+AllTrim(SC6->C6_SEUCOD)+"-ITEM "+AllTrim(SC6->C6_SEUDES)  // Informaï¿½ï¿½es Adicionais
											EndIf
										EndIf
					            	Else
										If !Empty(SC6->C6_SEUCOD)
						    	...
						    	If lVerPPB
						        	If SB1->B1_XXPROIN == "S"
											...
											_cPPB := AllTrim(SBM->BM_XDESPPB)
			   					   		...
			   					   		cAuxPPB := aProd[Len(aProd)][04]
							         	If Empty(_cPPB)   
								        	aProd[Len(aProd)][04] := AllTrim(SC6->C6_SEUDES)+'|'+AllTrim(SC6->C6_XXSHIPM)+'|'+Alltrim(SC6->C6_SEUCOD)+'|'+AllTrim(cAuxPPB)	// Descriï¿½ï¿½o Detalhada
								         	aProd[Len(aProd)][25] := ""	// Informacoes Adicionais
								      	Else
								         	aProd[Len(aProd)][04] := _cPPB // Descriï¿½ï¿½o do PPB
								         	aProd[Len(aProd)][25] := AllTrim(SC6->C6_SEUDES)+'|'+AllTrim(SC6->C6_XXSHIPM)+'|'+Alltrim(SC6->C6_SEUCOD)+'|'+AllTrim(cAuxPPB)  // Informaï¿½ï¿½es Adicionais
								      	EndIf
									Else
		       ...
		       EndIf   		
				...
				If lEECFAT
					...
					*/
						...
						PutMv("MV_EECFAT",.T.)
		                    //ï¿½Atualiza o peso e o volume da nota fiscal			 ï¿½
			                ...
			                //ï¿½Atualiza dados de exportaï¿½ï¿½o									ï¿½
			                //ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
			                U_GeraCDL(SF2->F2_DOC,SF2->F2_SERIE,SF2->F2_CLIENTE,SF2->F2_LOJA)
							       //CUSTOMIZAï¿½ï¿½O SIGAEEC DOMEX
				...
				If lEasy  .And. !Empty((cAliasSD1)->D1_TIPO_NF)
					...
					If !cTipoNF$"IPC" .And. cTipoNFEnt <> "6"
						...
						// DOMEX - mls  customizaï¿½ï¿½o  02/04/2015  posicao adi[1][34][3] numero do ato con drawback
						SC7->(DbSetOrder (1))
						SW4->(DbSetOrder (1))
					...
					Else
						...
						//DOMEX - mls  customizaï¿½ï¿½o  28/04/2015  para nfc posicao adi[1][34][3] numero do ato con drawback
						SC7->(DbSetOrder (1))
						SW4->(DbSetOrder (1))
						If SC7->(dbSeek( xFilial('SC7') + cPedido + cItemPC ))
						  _cPOEIC:=SC7->C7_PO_EIC
...
//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
//ï¿½CUSTOMIZACAO DOMEX - MARCO - MAS - OPUSVP                                      ï¿½
//ï¿½                                                                               ï¿½
//ï¿½Forca tratamento nas CFOPs de DEVOLUCAO para nnao Rejeitar a NF pelo Motivo 327ï¿½
//ï¿½CFOP de Devolucao para NF que nao tem Finalidade de Devolucao                  ï¿½
