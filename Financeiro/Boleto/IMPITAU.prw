...
//+----------------------------------------------------------------------------------------
User Function IMPITAU(cPrefixo,cNumero,cParcela,cCliente,cLoja,cBanco,cAgencia,cConta,nDias,cCarga,dEmissao)
	Local aRegs			:= {}
	Local aTamSX3			:= {}
	Local lEnd				:= .F.
	Local lAuto			:= .F.
...
Static Function CallLst()
	...
	If Select("IMPITAUA") > 0
		dbSelectArea("IMPITAUA")
		dbCloseArea()
	EndIf

	// Executa a select no banco para pegar os registros a processar
	TCQUERY cQry NEW ALIAS "IMPITAUA"
	dbSelectArea("IMPITAUA")
	dbGoTop()

	// Compatibiliza os campos com a TopField
	aTamSX3	:= TAMSX3("E1_EMISSAO")
	TCSETFIELD("IMPITAUA", "E1_EMISSAO",	aTamSX3[3], aTamSX3[1], aTamSX3[2])

	aTamSX3	:= TAMSX3("E1_VENCTO")
	TCSETFIELD("IMPITAUA", "E1_VENCTO",		aTamSX3[3], aTamSX3[1], aTamSX3[2])

	aTamSX3	:= TAMSX3("E1_VENCREA")
	TCSETFIELD("IMPITAUA", "E1_VENCREA",	aTamSX3[3], aTamSX3[1], aTamSX3[2])

	aTamSX3	:= TAMSX3("E1_VALOR")
	TCSETFIELD("IMPITAUA", "E1_VALOR",		aTamSX3[3], aTamSX3[1], aTamSX3[2])
	
	aTamSX3	:= TAMSX3("E1_XRENEG")
	TCSETFIELD("IMPITAUA", "E1_XRENEG",		aTamSX3[3], aTamSX3[1], aTamSX3[2])	

	// Conta os registros a serem processados
	IMPITAUA->( dbEval( { || nCnt++ },,{ || !Eof() } ) )
	dbGoTop()
	...
	While !Eof()
		IncProc( "Título: " + IMPITAUA->E1_PREFIXO +"/"+ IMPITAUA->E1_NUM +"/"+ IMPITAUA->E1_PARCELA )
